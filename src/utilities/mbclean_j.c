...
int main (int argc, char **argv) {
	...
	while ((c = getopt(argc, argv, "VvHhA:a:B:b:C:c:D:d:E:e:F:f:G:g:K:k:L:l:I:i:J:M:m:Q:q:P:p:R:r:S:s:T:t:U:u:W:w:X:x:Y:y:Zz")) != -1)
	  {
		switch (c)
		{
		case 'H':
	...
	while (read_data == MB_YES) {
		...
		if (oktoprocess == MB_YES) {
			...
			if ((status = mb_read_init( verbose,swathfileread,formatread,pings,lonflip,bounds, btime_i,etime_i,
			...
			if (verbose >= 0) {
				...
				fprintf(stderr,"%d spline spikes identified\n",nsmoothed);
