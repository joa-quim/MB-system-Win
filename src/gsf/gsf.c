...
int gsfOpen(const char *filename, const int mode, int *handle) {
	...
	*handle = fileTableIndex + 1;
	...
	if (stsize == 0) {
		...
		gsfFileTable[fileTableIndex].rec.header.version[GSF_VERSION_SIZE - 1] =
		    0; // changed "version[GSF_VERSION_SIZE]" to "version[GSF_VERSION_SIZE-1]" for MB-System DW Caress 21 March 2017
	...
	ret = sscanf(gsfFileTable[fileTableIndex].rec.header.version, "GSF-v%d.%d",
	             &gsfFileTable[fileTableIndex].major_version_number, &gsfFileTable[fileTableIndex].minor_version_number);
	if (ret != 2) {
		gsfError = GSF_UNRECOGNIZED_FILE;
		gsfClose((int)*handle);
		*handle = 0;
	 ...
	 */
	if ((mode == GSF_UPDATE) || (mode == GSF_UPDATE_INDEX) || (mode == GSF_CREATE)) {
...
int gsfRead(int handle, int desiredRecord, gsfDataID *dataID, gsfRecords *rptr, unsigned char *buf, int max_size) {
	int ret;
	gsfDataID tmpID;
	 ...
	 */
	if ((gsfFileTable[handle - 1].direct_access) && (desiredRecord != GSF_NEXT_RECORD)) {
...
static int gsfUnpackStream(int handle, int desiredRecord, gsfDataID *dataID, gsfRecords *rptr, unsigned char *buf, int max_size) {
	...
	while (readNext) {
		 ...
		 * flush the buffer.
		 */
		if ((gsfFileTable[handle - 1].update_flag) && (gsfFileTable[handle - 1].read_write_flag == LAST_OP_WRITE)) {
			if (fflush(gsfFileTable[handle - 1].fp)) {
				gsfError = GSF_FLUSH_ERROR;
				return (-1);
			}
		...
		if (readStat != 2) {
			if (feof(gsfFileTable[handle - 1].fp)) {
				 ...
				 *               reset file pointer
				 */
				fseek(gsfFileTable[handle - 1].fp, gsfFileTable[handle - 1].previous_record, SEEK_SET);
...
static int gsfSeekRecord(int handle, gsfDataID *id) {
	...
	if ((id->record_number < -1) || (id->record_number == 0) ||
	 ...
	 * the index record.
	 */
	addr = gsfFileTable[handle - 1].index_data.start_addr[id->recordID] + (long long)(offset * sizeof(INDEX_REC));
	...
	if (id->recordID == GSF_RECORD_SWATH_BATHYMETRY_PING) {
		...
		for (i = 1; i < gsfFileTable[handle - 1].index_data.number_of_records[0]; i++) {
			 ...
			 * prior to this one.
			 */
			if (gsfFileTable[handle - 1].index_data.scale_factor_addr[i].addr > index_rec.addr) {
				scale_index = i - 1;
				break;
			}
		}
...
int gsfWrite(int handle, gsfDataID *id, gsfRecords *rptr) {
	unsigned char *ucptr;
	gsfuLong tmpBuff[3] = {0, 0, 0};
	gsfuLong gsfBuff[3];
	gsfuLong temp;
	gsfuLong dataSize;
	size_t writeStat;
	...
	if (pad) {
		 ...
		 * old way.
		 */
		if ((gsfFileTable[handle - 1].major_version_number == 1) && (gsfFileTable[handle - 1].minor_version_number <= 2)) {
	 ...
	 *  flush the buffer.
	 */
	if ((gsfFileTable[handle - 1].update_flag) && (gsfFileTable[handle - 1].read_write_flag == LAST_OP_READ)) {
		if (fflush(gsfFileTable[handle - 1].fp)) {
			gsfError = GSF_FLUSH_ERROR;
			return (-1);
		}
...
int gsfIntError(void) { return gsfError; }
...
int gsfIndexTime(int handle, int record_type, int record_number, time_t *sec, long *nsec) {
	long long addr;
	int offset;
	INDEX_REC index_rec;
	 ...
	 *  index record.
	 */
	addr = gsfFileTable[handle - 1].index_data.start_addr[record_type] + (offset * sizeof(INDEX_REC));
	if (fseek(gsfFileTable[handle - 1].index_data.fp, addr, 0)) {
		gsfError = GSF_FILE_SEEK_ERROR;
		return (-1);
	}
...
int gsfCopyRecords(gsfRecords *target, const gsfRecords *source) {
	...
	if (source->mb_ping.nominal_depth != (double *)NULL) {
		if (target->mb_ping.nominal_depth == (double *)NULL) {
			...
			target->mb_ping.nominal_depth =
			    (double *)realloc(target->mb_ping.nominal_depth, sizeof(double) * source->mb_ping.number_beams);
			if (target->mb_ping.nominal_depth == (double *)NULL) {
				gsfError = GSF_MEMORY_ALLOCATION_FAILED;
				return (-1);
			}
	...
	if (source->mb_ping.across_track != (double *)NULL) {
		if (target->mb_ping.across_track == (double *)NULL) {
			...
			target->mb_ping.across_track =
			    (double *)realloc(target->mb_ping.across_track, sizeof(double) * source->mb_ping.number_beams);
			if (target->mb_ping.across_track == (double *)NULL) {
				gsfError = GSF_MEMORY_ALLOCATION_FAILED;
				return (-1);
			}
	...
	if (source->mb_ping.along_track != (double *)NULL) {
		if (target->mb_ping.along_track == (double *)NULL) {
			...
			target->mb_ping.along_track =
			    (double *)realloc(target->mb_ping.along_track, sizeof(double) * source->mb_ping.number_beams);
			if (target->mb_ping.along_track == (double *)NULL) {
				gsfError = GSF_MEMORY_ALLOCATION_FAILED;
				return (-1);
			}
	...
	if (source->mb_ping.travel_time != (double *)NULL) {
		if (target->mb_ping.travel_time == (double *)NULL) {
			...
			target->mb_ping.travel_time =
			    (double *)realloc(target->mb_ping.travel_time, sizeof(double) * source->mb_ping.number_beams);
			if (target->mb_ping.travel_time == (double *)NULL) {
				gsfError = GSF_MEMORY_ALLOCATION_FAILED;
				return (-1);
			}
	...
	if (source->mb_ping.beam_angle != (double *)NULL) {
		if (target->mb_ping.beam_angle == (double *)NULL) {
			...
			target->mb_ping.beam_angle =
			    (double *)realloc(target->mb_ping.beam_angle, sizeof(double) * source->mb_ping.number_beams);
			if (target->mb_ping.beam_angle == (double *)NULL) {
				gsfError = GSF_MEMORY_ALLOCATION_FAILED;
				return (-1);
			}
	...
	if (source->mb_ping.mc_amplitude != (double *)NULL) {
		if (target->mb_ping.mc_amplitude == (double *)NULL) {
			...
			target->mb_ping.mc_amplitude =
			    (double *)realloc(target->mb_ping.mc_amplitude, sizeof(double) * source->mb_ping.number_beams);
			if (target->mb_ping.mc_amplitude == (double *)NULL) {
				gsfError = GSF_MEMORY_ALLOCATION_FAILED;
				return (-1);
			}
	...
	if (source->mb_ping.mr_amplitude != (double *)NULL) {
		if (target->mb_ping.mr_amplitude == (double *)NULL) {
			...
			target->mb_ping.mr_amplitude =
			    (double *)realloc(target->mb_ping.mr_amplitude, sizeof(double) * source->mb_ping.number_beams);
			if (target->mb_ping.mr_amplitude == (double *)NULL) {
				gsfError = GSF_MEMORY_ALLOCATION_FAILED;
				return (-1);
			}
	...
	if (source->mb_ping.echo_width != (double *)NULL) {
		if (target->mb_ping.echo_width == (double *)NULL) {
			...
			target->mb_ping.echo_width =
			    (double *)realloc(target->mb_ping.echo_width, sizeof(double) * source->mb_ping.number_beams);
			if (target->mb_ping.echo_width == (double *)NULL) {
				gsfError = GSF_MEMORY_ALLOCATION_FAILED;
				return (-1);
			}
	...
	if (source->mb_ping.quality_factor != (double *)NULL) {
		if (target->mb_ping.quality_factor == (double *)NULL) {
			...
			target->mb_ping.quality_factor =
			    (double *)realloc(target->mb_ping.quality_factor, sizeof(double) * source->mb_ping.number_beams);
			if (target->mb_ping.quality_factor == (double *)NULL) {
				gsfError = GSF_MEMORY_ALLOCATION_FAILED;
				return (-1);
			}
	...
	if (source->mb_ping.receive_heave != (double *)NULL) {
		if (target->mb_ping.receive_heave == (double *)NULL) {
			...
			target->mb_ping.receive_heave =
			    (double *)realloc(target->mb_ping.receive_heave, sizeof(double) * source->mb_ping.number_beams);
			if (target->mb_ping.receive_heave == (double *)NULL) {
				gsfError = GSF_MEMORY_ALLOCATION_FAILED;
				return (-1);
			}
	...
	if (source->mb_ping.depth_error != (double *)NULL) {
		if (target->mb_ping.depth_error == (double *)NULL) {
			...
			target->mb_ping.depth_error =
			    (double *)realloc(target->mb_ping.depth_error, sizeof(double) * source->mb_ping.number_beams);
			if (target->mb_ping.depth_error == (double *)NULL) {
				gsfError = GSF_MEMORY_ALLOCATION_FAILED;
				return (-1);
			}
	...
	if (source->mb_ping.across_track_error != (double *)NULL) {
		if (target->mb_ping.across_track_error == (double *)NULL) {
			...
			target->mb_ping.across_track_error =
			    (double *)realloc(target->mb_ping.across_track_error, sizeof(double) * source->mb_ping.number_beams);
			if (target->mb_ping.across_track_error == (double *)NULL) {
				gsfError = GSF_MEMORY_ALLOCATION_FAILED;
				return (-1);
			}
		}
		memcpy(target->mb_ping.across_track_error, source->mb_ping.across_track_error,
		       sizeof(double) * source->mb_ping.number_beams);
	}
	...
	if (source->mb_ping.along_track_error != (double *)NULL) {
		if (target->mb_ping.along_track_error == (double *)NULL) {
			...
			target->mb_ping.along_track_error =
			    (double *)realloc(target->mb_ping.along_track_error, sizeof(double) * source->mb_ping.number_beams);
			if (target->mb_ping.along_track_error == (double *)NULL) {
				gsfError = GSF_MEMORY_ALLOCATION_FAILED;
				return (-1);
			}
		}
		memcpy(target->mb_ping.along_track_error, source->mb_ping.along_track_error,
		       sizeof(double) * source->mb_ping.number_beams);
	}
	...
	if (source->mb_ping.quality_flags != (unsigned char *)NULL) {
		if (target->mb_ping.quality_flags == (unsigned char *)NULL) {
			...
			target->mb_ping.quality_flags =
			    (unsigned char *)realloc(target->mb_ping.quality_flags, sizeof(unsigned char) * source->mb_ping.number_beams);
			if (target->mb_ping.quality_flags == (unsigned char *)NULL) {
				gsfError = GSF_MEMORY_ALLOCATION_FAILED;
				return (-1);
			}
		}
		memcpy(target->mb_ping.quality_flags, source->mb_ping.quality_flags,
		       sizeof(unsigned char) * source->mb_ping.number_beams);
	}
	...
	if (source->mb_ping.beam_flags != (unsigned char *)NULL) {
		if (target->mb_ping.beam_flags == (unsigned char *)NULL) {
			...
			target->mb_ping.beam_flags =
			    (unsigned char *)realloc(target->mb_ping.beam_flags, sizeof(unsigned char) * source->mb_ping.number_beams);
			if (target->mb_ping.beam_flags == (unsigned char *)NULL) {
				gsfError = GSF_MEMORY_ALLOCATION_FAILED;
				return (-1);
			}
	...
	if (source->mb_ping.signal_to_noise != (double *)NULL) {
		if (target->mb_ping.signal_to_noise == (double *)NULL) {
			...
			target->mb_ping.signal_to_noise =
			    (double *)realloc(target->mb_ping.signal_to_noise, sizeof(double) * source->mb_ping.number_beams);
			if (target->mb_ping.signal_to_noise == (double *)NULL) {
				gsfError = GSF_MEMORY_ALLOCATION_FAILED;
				return (-1);
			}
	...
	if (source->mb_ping.beam_angle_forward != (double *)NULL) {
		if (target->mb_ping.beam_angle_forward == (double *)NULL) {
			...
			target->mb_ping.beam_angle_forward =
			    (double *)realloc(target->mb_ping.beam_angle_forward, sizeof(double) * source->mb_ping.number_beams);
			if (target->mb_ping.beam_angle_forward == (double *)NULL) {
				gsfError = GSF_MEMORY_ALLOCATION_FAILED;
				return (-1);
			}
		}
		memcpy(target->mb_ping.beam_angle_forward, source->mb_ping.beam_angle_forward,
		       sizeof(double) * source->mb_ping.number_beams);
	}
	...
	if (source->mb_ping.vertical_error != (double *)NULL) {
		if (target->mb_ping.vertical_error == (double *)NULL) {
			...
			target->mb_ping.vertical_error =
			    (double *)realloc(target->mb_ping.vertical_error, sizeof(double) * source->mb_ping.number_beams);
			if (target->mb_ping.vertical_error == (double *)NULL) {
				gsfError = GSF_MEMORY_ALLOCATION_FAILED;
				return (-1);
			}
	...
	if (source->mb_ping.horizontal_error != (double *)NULL) {
		if (target->mb_ping.horizontal_error == (double *)NULL) {
			...
			target->mb_ping.horizontal_error =
			    (double *)realloc(target->mb_ping.horizontal_error, sizeof(double) * source->mb_ping.number_beams);
			if (target->mb_ping.horizontal_error == (double *)NULL) {
				gsfError = GSF_MEMORY_ALLOCATION_FAILED;
				return (-1);
			}
	...
	if (source->mb_ping.sector_number != (unsigned short *)NULL) {
		if (target->mb_ping.sector_number == (unsigned short *)NULL) {
			...
			target->mb_ping.sector_number =
			    (unsigned short *)realloc(target->mb_ping.sector_number, sizeof(unsigned short) * source->mb_ping.number_beams);
			if (target->mb_ping.sector_number == (unsigned short *)NULL) {
				gsfError = GSF_MEMORY_ALLOCATION_FAILED;
				return (-1);
			}
		}
		memcpy(target->mb_ping.sector_number, source->mb_ping.sector_number,
		       sizeof(unsigned short) * source->mb_ping.number_beams);
	}
	...
	if (source->mb_ping.detection_info != (unsigned short *)NULL) {
		if (target->mb_ping.detection_info == (unsigned short *)NULL) {
			...
			target->mb_ping.detection_info =
			    (unsigned short *)realloc(target->mb_ping.detection_info, sizeof(unsigned short) * source->mb_ping.number_beams);
			if (target->mb_ping.detection_info == (unsigned short *)NULL) {
				gsfError = GSF_MEMORY_ALLOCATION_FAILED;
				return (-1);
			}
		}
		memcpy(target->mb_ping.detection_info, source->mb_ping.detection_info,
		       sizeof(unsigned short) * source->mb_ping.number_beams);
	}
	...
	if (source->mb_ping.incident_beam_adj != (double *)NULL) {
		if (target->mb_ping.incident_beam_adj == (double *)NULL) {
			...
			target->mb_ping.incident_beam_adj =
			    (double *)realloc(target->mb_ping.incident_beam_adj, sizeof(double) * source->mb_ping.number_beams);
			if (target->mb_ping.incident_beam_adj == (double *)NULL) {
				gsfError = GSF_MEMORY_ALLOCATION_FAILED;
				return (-1);
			}
		}
		memcpy(target->mb_ping.incident_beam_adj, source->mb_ping.incident_beam_adj,
		       sizeof(double) * source->mb_ping.number_beams);
	}
	...
	if (source->mb_ping.system_cleaning != (unsigned short *)NULL) {
		if (target->mb_ping.system_cleaning == (unsigned short *)NULL) {
			...
			target->mb_ping.system_cleaning =
			    (unsigned short *)realloc(target->mb_ping.system_cleaning, sizeof(unsigned short) * source->mb_ping.number_beams);
			if (target->mb_ping.system_cleaning == (unsigned short *)NULL) {
				gsfError = GSF_MEMORY_ALLOCATION_FAILED;
				return (-1);
			}
		}
		memcpy(target->mb_ping.system_cleaning, source->mb_ping.system_cleaning,
		       sizeof(unsigned short) * source->mb_ping.number_beams);
	}
	...
	if (source->mb_ping.doppler_corr != (double *)NULL) {
		if (target->mb_ping.doppler_corr == (double *)NULL) {
			...
			target->mb_ping.doppler_corr =
			    (double *)realloc(target->mb_ping.doppler_corr, sizeof(double) * source->mb_ping.number_beams);
			if (target->mb_ping.doppler_corr == (double *)NULL) {
				gsfError = GSF_MEMORY_ALLOCATION_FAILED;
				return (-1);
			}
	...
	if (source->mb_ping.brb_inten != (gsfBRBIntensity *)NULL) {
		...
		if (source->mb_ping.brb_inten->time_series != (gsfTimeSeriesIntensity *)NULL) {
			if (target->mb_ping.brb_inten->time_series == (gsfTimeSeriesIntensity *)NULL) {
				target->mb_ping.brb_inten->time_series =
				    (gsfTimeSeriesIntensity *)calloc(sizeof(gsfTimeSeriesIntensity), source->mb_ping.number_beams);
				if (target->mb_ping.brb_inten->time_series == (gsfTimeSeriesIntensity *)NULL) {
					gsfError = GSF_MEMORY_ALLOCATION_FAILED;
					return (-1);
				}
				...
				target->mb_ping.brb_inten->time_series = (gsfTimeSeriesIntensity *)realloc(
				    target->mb_ping.brb_inten->time_series, sizeof(gsfTimeSeriesIntensity) * source->mb_ping.number_beams);
				if (target->mb_ping.brb_inten->time_series == (gsfTimeSeriesIntensity *)NULL) {
					gsfError = GSF_MEMORY_ALLOCATION_FAILED;
					return (-1);
				}
			...
			for (i = 0; i < source->mb_ping.number_beams; i++) {
				if (source->mb_ping.brb_inten->time_series[i].samples != (unsigned int *)NULL) {
					if (target->mb_ping.brb_inten->time_series[i].samples == (unsigned int *)NULL) {
						target->mb_ping.brb_inten->time_series[i].samples =
						    (unsigned int *)calloc(sizeof(unsigned int), source->mb_ping.brb_inten->time_series[i].sample_count);
						if (target->mb_ping.brb_inten->time_series[i].samples == (unsigned int *)NULL) {
							gsfError = GSF_MEMORY_ALLOCATION_FAILED;
							return (-1);
						}
					}
					else if (target->mb_ping.brb_inten->time_series[i].sample_count <
					         source->mb_ping.brb_inten->time_series[i].sample_count) {
						target->mb_ping.brb_inten->time_series[i].samples = (unsigned int *)realloc(
						    target->mb_ping.brb_inten->time_series[i].samples,
						    sizeof(unsigned int) * source->mb_ping.brb_inten->time_series[i].sample_count);
						if (target->mb_ping.brb_inten->time_series[i].samples == (unsigned int *)NULL) {
							gsfError = GSF_MEMORY_ALLOCATION_FAILED;
							return (-1);
						}
					}
					memcpy(target->mb_ping.brb_inten->time_series[i].samples, source->mb_ping.brb_inten->time_series[i].samples,
					       sizeof(unsigned int) * source->mb_ping.brb_inten->time_series[i].sample_count);
					target->mb_ping.brb_inten->time_series[i].sample_count =
					    source->mb_ping.brb_inten->time_series[i].sample_count;
					target->mb_ping.brb_inten->time_series[i].detect_sample =
					    source->mb_ping.brb_inten->time_series[i].detect_sample;
				}
			}
		}
	}
	...
	for (i = 0; i < source->process_parameters.number_parameters; i++) {
		if (target->process_parameters.param[i] == (char *)NULL) {
			...
			strncpy(target->process_parameters.param[i], source->process_parameters.param[i],
			        source->process_parameters.param_size[i] + 1);
			...
			target->process_parameters.param[i] =
			    (char *)realloc(target->process_parameters.param[i], source->process_parameters.param_size[i] + 1);
			if (target->process_parameters.param[i] == (char *)NULL) {
				gsfError = GSF_MEMORY_ALLOCATION_FAILED;
				return (-1);
			}
	...
	for (i = 0; i < source->sensor_parameters.number_parameters; i++) {
		if (target->sensor_parameters.param[i] == (char *)NULL) {
			...
			strncpy(target->sensor_parameters.param[i], source->sensor_parameters.param[i],
			        source->sensor_parameters.param_size[i] + 1);
			...
			target->sensor_parameters.param[i] =
			    (char *)realloc(target->sensor_parameters.param[i], source->sensor_parameters.param_size[i] + 1);
			if (target->sensor_parameters.param[i] == (char *)NULL) {
				gsfError = GSF_MEMORY_ALLOCATION_FAILED;
				return (-1);
			}
	...
	if (target->attitude.attitude_time == (struct timespec *)NULL) {
		...
		memcpy(target->attitude.attitude_time, source->attitude.attitude_time,
		       sizeof(struct timespec) * source->attitude.num_measurements);
	}
	else if (target->attitude.num_measurements < source->attitude.num_measurements) {
		target->attitude.attitude_time = (struct timespec *)realloc(target->attitude.attitude_time,
		                                                            sizeof(struct timespec) * source->attitude.num_measurements);
		if (target->attitude.attitude_time == (struct timespec *)NULL) {
			gsfError = GSF_MEMORY_ALLOCATION_FAILED;
			return (-1);
	...
	if (target->attitude.heading == (double *)NULL) {
		...
		target->attitude.heading =
		    (double *)realloc(target->attitude.heading, sizeof(double) * source->attitude.num_measurements);
		if (target->attitude.heading == (double *)NULL) {
			gsfError = GSF_MEMORY_ALLOCATION_FAILED;
			return (-1);
		}
...
static int gsfSetParam(int handle, int index, char *val, gsfRecords *rec) {
	...
	/* If memory has already been allocated, make sure we have enough space */
		 ...
		 */
		if ((gsfFileTable[handle - 1].access_mode == GSF_UPDATE) || (gsfFileTable[handle - 1].access_mode == GSF_UPDATE_INDEX)) {
...
int gsfPutMBParams(const gsfMBParams *p, gsfRecords *rec, int handle, int numArrays) {
	...
	if (num_tx == 1) {
		if (p->to_apply.draft[0] == GSF_UNKNOWN_PARAM_VALUE) {
			sprintf(temp, "DRAFT_TO_APPLY=%s", GSF_UNKNOWN_PARAM_TEXT);
		}
		else if ((p->to_apply.draft[0] > GSF_MIN_PARAM) && (p->to_apply.draft[0] < GSF_MAX_PARAM)) {
			sprintf(temp, "DRAFT_TO_APPLY=%+06.2f", p->to_apply.draft[0]);
		}
		else {
			gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
			return (-1);
	...
	if (num_tx == 1) {
		if (p->to_apply.pitch_bias[0] == GSF_UNKNOWN_PARAM_VALUE) {
			sprintf(temp, "PITCH_TO_APPLY=%s", GSF_UNKNOWN_PARAM_TEXT);
		}
		else if ((p->to_apply.pitch_bias[0] > GSF_MIN_PARAM) && (p->to_apply.pitch_bias[0] < GSF_MAX_PARAM)) {
			sprintf(temp, "PITCH_TO_APPLY=%+06.2f", p->to_apply.pitch_bias[0]);
		}
		else {
			gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
			return (-1);
	...
	if (num_tx == 1) {
		if (p->to_apply.roll_bias[0] == GSF_UNKNOWN_PARAM_VALUE) {
			sprintf(temp, "ROLL_TO_APPLY=%s", GSF_UNKNOWN_PARAM_TEXT);
		}
		else if ((p->to_apply.roll_bias[0] > GSF_MIN_PARAM) && (p->to_apply.roll_bias[0] < GSF_MAX_PARAM)) {
			sprintf(temp, "ROLL_TO_APPLY=%+06.2f", p->to_apply.roll_bias[0]);
		}
		else {
			gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
			return (-1);
	...
	if (num_tx == 1) {
		if (p->to_apply.gyro_bias[0] == GSF_UNKNOWN_PARAM_VALUE) {
			sprintf(temp, "GYRO_TO_APPLY=%s", GSF_UNKNOWN_PARAM_TEXT);
		}
		else if ((p->to_apply.gyro_bias[0] > GSF_MIN_PARAM) && (p->to_apply.gyro_bias[0] < GSF_MAX_PARAM)) {
			sprintf(temp, "GYRO_TO_APPLY=%+06.2f", p->to_apply.gyro_bias[0]);
		}
		else {
			gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
			return (-1);
	...
	if (p->to_apply.position_x_offset == GSF_UNKNOWN_PARAM_VALUE) {
		...
		sprintf(temp2, "%+06.2f,", p->to_apply.position_x_offset);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (p->to_apply.position_y_offset == GSF_UNKNOWN_PARAM_VALUE) {
		...
		sprintf(temp2, "%+06.2f,", p->to_apply.position_y_offset);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (p->to_apply.position_z_offset == GSF_UNKNOWN_PARAM_VALUE) {
		...
		sprintf(temp2, "%+06.2f", p->to_apply.position_z_offset);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (p->to_apply.antenna_x_offset == GSF_UNKNOWN_PARAM_VALUE) {
		...
		sprintf(temp2, "%+06.2f,", p->to_apply.antenna_x_offset);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (p->to_apply.antenna_y_offset == GSF_UNKNOWN_PARAM_VALUE) {
		...
		sprintf(temp2, "%+06.2f,", p->to_apply.antenna_y_offset);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (p->to_apply.antenna_z_offset == GSF_UNKNOWN_PARAM_VALUE) {
		...
		sprintf(temp2, "%+06.2f", p->to_apply.antenna_z_offset);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (num_tx == 1) {
		if (p->to_apply.transducer_x_offset[0] == GSF_UNKNOWN_PARAM_VALUE) {
			...
			sprintf(temp2, "%+06.2f,", p->to_apply.transducer_x_offset[0]);
		}
		else {
			gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
			return (-1);
		...
		if (p->to_apply.transducer_y_offset[0] == GSF_UNKNOWN_PARAM_VALUE) {
			...
			sprintf(temp2, "%+06.2f,", p->to_apply.transducer_y_offset[0]);
		}
		else {
			gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
			return (-1);
		...
		if (p->to_apply.transducer_z_offset[0] == GSF_UNKNOWN_PARAM_VALUE) {
			...
			sprintf(temp2, "%+06.2f", p->to_apply.transducer_z_offset[0]);
		}
		else {
			gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
			return (-1);
		...
		if (p->to_apply.transducer_x_offset[1] == GSF_UNKNOWN_PARAM_VALUE) {
			...
			sprintf(temp2, "%+06.2f,", p->to_apply.transducer_x_offset[1]);
		}
		else {
			gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
			return (-1);
		...
		if (p->to_apply.transducer_y_offset[1] == GSF_UNKNOWN_PARAM_VALUE) {
			...
			sprintf(temp2, "%+06.2f,", p->to_apply.transducer_y_offset[1]);
		}
		else {
			gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
			return (-1);
		...
		if (p->to_apply.transducer_z_offset[1] == GSF_UNKNOWN_PARAM_VALUE) {
			...
			sprintf(temp2, "%+06.2f", p->to_apply.transducer_z_offset[1]);
		}
		else {
			gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
			return (-1);
	...
	if (num_tx == 1) {
		...
		else if ((p->to_apply.transducer_pitch_offset[0] > GSF_MIN_PARAM) &&
		         (p->to_apply.transducer_pitch_offset[0] < GSF_MAX_PARAM)) {
			sprintf(temp, "TRANSDUCER_PITCH_OFFSET_TO_APPLY=%+06.2f", p->to_apply.transducer_pitch_offset[0]);
		}
		else {
			gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
			return (-1);
		...
		else if ((p->to_apply.transducer_pitch_offset[1] > GSF_MIN_PARAM) &&
		         (p->to_apply.transducer_pitch_offset[1] < GSF_MAX_PARAM)) {
	...
	if (num_tx == 1) {
		...
		else if ((p->to_apply.transducer_roll_offset[0] > GSF_MIN_PARAM) &&
		         (p->to_apply.transducer_roll_offset[0] < GSF_MAX_PARAM)) {
			sprintf(temp, "TRANSDUCER_ROLL_OFFSET_TO_APPLY=%+06.2f", p->to_apply.transducer_roll_offset[0]);
		}
		else {
			gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
			return (-1);
		...
		else if ((p->to_apply.transducer_roll_offset[1] > GSF_MIN_PARAM) &&
		         (p->to_apply.transducer_roll_offset[1] < GSF_MAX_PARAM)) {
	...
	if (num_tx == 1) {
		if (p->to_apply.transducer_heading_offset[0] == GSF_UNKNOWN_PARAM_VALUE) {
			sprintf(temp, "TRANSDUCER_HEADING_OFFSET_TO_APPLY=%s", GSF_UNKNOWN_PARAM_TEXT);
		}
		else if ((p->to_apply.transducer_heading_offset[0] > GSF_MIN_PARAM) &&
		         (p->to_apply.transducer_heading_offset[0] < GSF_MAX_PARAM)) {
			sprintf(temp, "TRANSDUCER_HEADING_OFFSET_TO_APPLY=%+06.2f", p->to_apply.transducer_heading_offset[0]);
		}
		else {
			gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
			return (-1);
		...
		else if ((p->to_apply.transducer_heading_offset[1] > GSF_MIN_PARAM) &&
		         (p->to_apply.transducer_heading_offset[1] < GSF_MAX_PARAM)) {
	...
	if (p->to_apply.mru_pitch_bias == GSF_UNKNOWN_PARAM_VALUE) {
		sprintf(temp, "MRU_PITCH_TO_APPLY=%s", GSF_UNKNOWN_PARAM_TEXT);
	}
	else if ((p->to_apply.mru_pitch_bias > GSF_MIN_PARAM) && (p->to_apply.mru_pitch_bias < GSF_MAX_PARAM)) {
		sprintf(temp, "MRU_PITCH_TO_APPLY=%+06.2f", p->to_apply.mru_pitch_bias);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (p->to_apply.mru_roll_bias == GSF_UNKNOWN_PARAM_VALUE) {
		sprintf(temp, "MRU_ROLL_TO_APPLY=%s", GSF_UNKNOWN_PARAM_TEXT);
	}
	else if ((p->to_apply.mru_roll_bias > GSF_MIN_PARAM) && (p->to_apply.mru_roll_bias < GSF_MAX_PARAM)) {
		sprintf(temp, "MRU_ROLL_TO_APPLY=%+06.2f", p->to_apply.mru_roll_bias);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (p->to_apply.mru_heading_bias == GSF_UNKNOWN_PARAM_VALUE) {
		sprintf(temp, "MRU_HEADING_TO_APPLY=%s", GSF_UNKNOWN_PARAM_TEXT);
	}
	else if ((p->to_apply.mru_heading_bias > GSF_MIN_PARAM) && (p->to_apply.mru_heading_bias < GSF_MAX_PARAM)) {
		sprintf(temp, "MRU_HEADING_TO_APPLY=%+06.2f", p->to_apply.mru_heading_bias);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (p->to_apply.mru_x_offset == GSF_UNKNOWN_PARAM_VALUE) {
		...
		sprintf(temp2, "%+06.2f,", p->to_apply.mru_x_offset);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (p->to_apply.mru_y_offset == GSF_UNKNOWN_PARAM_VALUE) {
		...
		sprintf(temp2, "%+06.2f,", p->to_apply.mru_y_offset);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (p->to_apply.mru_z_offset == GSF_UNKNOWN_PARAM_VALUE) {
		...
		sprintf(temp2, "%+06.2f", p->to_apply.mru_z_offset);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	else if ((p->to_apply.center_of_rotation_x_offset > GSF_MIN_PARAM) &&
	         (p->to_apply.center_of_rotation_x_offset < GSF_MAX_PARAM)) {
		sprintf(temp2, "%+06.2f,", p->to_apply.center_of_rotation_x_offset);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	else if ((p->to_apply.center_of_rotation_y_offset > GSF_MIN_PARAM) &&
	         (p->to_apply.center_of_rotation_y_offset < GSF_MAX_PARAM)) {
		sprintf(temp2, "%+06.2f,", p->to_apply.center_of_rotation_y_offset);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	else if ((p->to_apply.center_of_rotation_z_offset > GSF_MIN_PARAM) &&
	         (p->to_apply.center_of_rotation_z_offset < GSF_MAX_PARAM)) {
		sprintf(temp2, "%+06.2f", p->to_apply.center_of_rotation_z_offset);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (p->to_apply.position_latency == GSF_UNKNOWN_PARAM_VALUE) {
		sprintf(temp, "POSITION_LATENCY_TO_APPLY=%s", GSF_UNKNOWN_PARAM_TEXT);
	}
	else if ((p->to_apply.position_latency > GSF_MIN_PARAM) && (p->to_apply.position_latency < GSF_MAX_PARAM)) {
		sprintf(temp, "POSITION_LATENCY_TO_APPLY=%+06.3f", p->to_apply.position_latency);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (p->to_apply.attitude_latency == GSF_UNKNOWN_PARAM_VALUE) {
		sprintf(temp, "ATTITUDE_LATENCY_TO_APPLY=%s", GSF_UNKNOWN_PARAM_TEXT);
	}
	else if ((p->to_apply.attitude_latency > GSF_MIN_PARAM) && (p->to_apply.attitude_latency < GSF_MAX_PARAM)) {
		sprintf(temp, "ATTITUDE_LATENCY_TO_APPLY=%+06.3f", p->to_apply.attitude_latency);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (p->to_apply.depth_sensor_latency == GSF_UNKNOWN_PARAM_VALUE) {
		sprintf(temp, "DEPTH_SENSOR_LATENCY_TO_APPLY=%s", GSF_UNKNOWN_PARAM_TEXT);
	}
	else if ((p->to_apply.depth_sensor_latency > GSF_MIN_PARAM) && (p->to_apply.depth_sensor_latency < GSF_MAX_PARAM)) {
		sprintf(temp, "DEPTH_SENSOR_LATENCY_TO_APPLY=%+06.3f", p->to_apply.depth_sensor_latency);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (p->to_apply.depth_sensor_x_offset == GSF_UNKNOWN_PARAM_VALUE) {
		...
		sprintf(temp, "DEPTH_SENSOR_OFFSET_TO_APPLY=%+06.2f,", p->to_apply.depth_sensor_x_offset);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (p->to_apply.depth_sensor_y_offset == GSF_UNKNOWN_PARAM_VALUE) {
		...
		sprintf(temp2, "%+06.2f,", p->to_apply.depth_sensor_y_offset);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (num_rx == 1) {
		...
		else if ((p->to_apply.rx_transducer_x_offset[0] > GSF_MIN_PARAM) &&
		         (p->to_apply.rx_transducer_x_offset[0] < GSF_MAX_PARAM)) {
		...
		else if ((p->to_apply.rx_transducer_y_offset[0] > GSF_MIN_PARAM) &&
		         (p->to_apply.rx_transducer_y_offset[0] < GSF_MAX_PARAM)) {
		...
		else if ((p->to_apply.rx_transducer_z_offset[0] > GSF_MIN_PARAM) &&
		         (p->to_apply.rx_transducer_z_offset[0] < GSF_MAX_PARAM)) {
		...
		else if ((p->to_apply.rx_transducer_x_offset[1] > GSF_MIN_PARAM) &&
		         (p->to_apply.rx_transducer_x_offset[1] < GSF_MAX_PARAM)) {
		...
		else if ((p->to_apply.rx_transducer_y_offset[1] > GSF_MIN_PARAM) &&
		         (p->to_apply.rx_transducer_y_offset[1] < GSF_MAX_PARAM)) {
		...
		else if ((p->to_apply.rx_transducer_z_offset[1] > GSF_MIN_PARAM) &&
		         (p->to_apply.rx_transducer_z_offset[1] < GSF_MAX_PARAM)) {
	...
	if (num_rx == 1) {
		...
		else if ((p->to_apply.rx_transducer_pitch_offset[0] > GSF_MIN_PARAM) &&
		         (p->to_apply.rx_transducer_pitch_offset[0] < GSF_MAX_PARAM)) {
		...
		else if ((p->to_apply.rx_transducer_pitch_offset[1] > GSF_MIN_PARAM) &&
		         (p->to_apply.rx_transducer_pitch_offset[1] < GSF_MAX_PARAM)) {
	...
	if (num_rx == 1) {
		...
		else if ((p->to_apply.rx_transducer_roll_offset[0] > GSF_MIN_PARAM) &&
		         (p->to_apply.rx_transducer_roll_offset[0] < GSF_MAX_PARAM)) {
		...
		else if ((p->to_apply.rx_transducer_roll_offset[1] > GSF_MIN_PARAM) &&
		         (p->to_apply.rx_transducer_roll_offset[1] < GSF_MAX_PARAM)) {
	...
	if (num_rx == 1) {
		...
		else if ((p->to_apply.rx_transducer_heading_offset[0] > GSF_MIN_PARAM) &&
		         (p->to_apply.rx_transducer_heading_offset[0] < GSF_MAX_PARAM)) {
		...
		else if ((p->to_apply.rx_transducer_heading_offset[1] > GSF_MIN_PARAM) &&
		         (p->to_apply.rx_transducer_heading_offset[1] < GSF_MAX_PARAM)) {
	...
	if (p->applied.position_x_offset == GSF_UNKNOWN_PARAM_VALUE) {
		...
		sprintf(temp2, "%+06.2f,", p->applied.position_x_offset);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (p->applied.position_y_offset == GSF_UNKNOWN_PARAM_VALUE) {
		...
		sprintf(temp2, "%+06.2f,", p->applied.position_y_offset);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (p->applied.position_z_offset == GSF_UNKNOWN_PARAM_VALUE) {
		...
		sprintf(temp2, "%+06.2f", p->applied.position_z_offset);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (p->applied.antenna_x_offset == GSF_UNKNOWN_PARAM_VALUE) {
		...
		sprintf(temp2, "%+06.2f,", p->applied.antenna_x_offset);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (p->applied.antenna_y_offset == GSF_UNKNOWN_PARAM_VALUE) {
		...
		sprintf(temp2, "%+06.2f,", p->applied.antenna_y_offset);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (p->applied.antenna_z_offset == GSF_UNKNOWN_PARAM_VALUE) {
		...
		sprintf(temp2, "%+06.2f", p->applied.antenna_z_offset);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (num_tx == 1) {
		if (p->applied.transducer_x_offset[0] == GSF_UNKNOWN_PARAM_VALUE) {
			...
			sprintf(temp2, "%+06.2f,", p->applied.transducer_x_offset[0]);
		}
		else {
			gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
			return (-1);
		...
		if (p->applied.transducer_y_offset[0] == GSF_UNKNOWN_PARAM_VALUE) {
			...
			sprintf(temp2, "%+06.2f,", p->applied.transducer_y_offset[0]);
		}
		else {
			gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
			return (-1);
		...
		if (p->applied.transducer_z_offset[0] == GSF_UNKNOWN_PARAM_VALUE) {
			...
			sprintf(temp2, "%+06.2f", p->applied.transducer_z_offset[0]);
		}
		else {
			gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
			return (-1);
		...
		if (p->applied.transducer_x_offset[1] == GSF_UNKNOWN_PARAM_VALUE) {
			...
			sprintf(temp2, "%+06.2f,", p->applied.transducer_x_offset[1]);
		}
		else {
			gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
			return (-1);
		...
		if (p->applied.transducer_y_offset[1] == GSF_UNKNOWN_PARAM_VALUE) {
			...
			sprintf(temp2, "%+06.2f,", p->applied.transducer_y_offset[1]);
		}
		else {
			gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
			return (-1);
		...
		if (p->applied.transducer_z_offset[1] == GSF_UNKNOWN_PARAM_VALUE) {
			...
			sprintf(temp2, "%+06.2f", p->applied.transducer_z_offset[1]);
		}
		else {
			gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
			return (-1);
	...
	if (num_tx == 1) {
		if (p->applied.transducer_pitch_offset[0] == GSF_UNKNOWN_PARAM_VALUE) {
			sprintf(temp, "APPLIED_TRANSDUCER_PITCH_OFFSET=%s", GSF_UNKNOWN_PARAM_TEXT);
		}
		else if ((p->applied.transducer_pitch_offset[0] > GSF_MIN_PARAM) &&
		         (p->applied.transducer_pitch_offset[0] < GSF_MAX_PARAM)) {
			sprintf(temp, "APPLIED_TRANSDUCER_PITCH_OFFSET=%+06.2f", p->applied.transducer_pitch_offset[0]);
		}
		else {
			gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
			return (-1);
		...
		else if ((p->applied.transducer_pitch_offset[1] > GSF_MIN_PARAM) &&
		         (p->applied.transducer_pitch_offset[1] < GSF_MAX_PARAM)) {
	...
	if (num_tx == 1) {
		if (p->applied.transducer_roll_offset[0] == GSF_UNKNOWN_PARAM_VALUE) {
			sprintf(temp, "APPLIED_TRANSDUCER_ROLL_OFFSET=%s", GSF_UNKNOWN_PARAM_TEXT);
		}
		else if ((p->applied.transducer_roll_offset[0] > GSF_MIN_PARAM) &&
		         (p->applied.transducer_roll_offset[0] < GSF_MAX_PARAM)) {
			sprintf(temp, "APPLIED_TRANSDUCER_ROLL_OFFSET=%+06.2f", p->applied.transducer_roll_offset[0]);
		}
		else {
			gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
			return (-1);
		...
		else if ((p->applied.transducer_roll_offset[1] > GSF_MIN_PARAM) &&
		         (p->applied.transducer_roll_offset[1] < GSF_MAX_PARAM)) {
	...
	if (num_tx == 1) {
		if (p->applied.transducer_heading_offset[0] == GSF_UNKNOWN_PARAM_VALUE) {
			sprintf(temp, "APPLIED_TRANSDUCER_HEADING_OFFSET=%s", GSF_UNKNOWN_PARAM_TEXT);
		}
		else if ((p->applied.transducer_heading_offset[0] > GSF_MIN_PARAM) &&
		         (p->applied.transducer_heading_offset[0] < GSF_MAX_PARAM)) {
			sprintf(temp, "APPLIED_TRANSDUCER_HEADING_OFFSET=%+06.2f", p->applied.transducer_heading_offset[0]);
		}
		else {
			gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
			return (-1);
		...
		else if ((p->applied.transducer_heading_offset[1] > GSF_MIN_PARAM) &&
		         (p->applied.transducer_heading_offset[1] < GSF_MAX_PARAM)) {
	...
	if (p->applied.mru_roll_bias == GSF_UNKNOWN_PARAM_VALUE) {
		sprintf(temp, "APPLIED_MRU_ROLL=%s", GSF_UNKNOWN_PARAM_TEXT);
	}
	else if ((p->applied.mru_roll_bias > GSF_MIN_PARAM) && (p->applied.mru_roll_bias < GSF_MAX_PARAM)) {
		sprintf(temp, "APPLIED_MRU_ROLL=%+06.2f", p->applied.mru_roll_bias);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (p->applied.mru_pitch_bias == GSF_UNKNOWN_PARAM_VALUE) {
		sprintf(temp, "APPLIED_MRU_PITCH=%s", GSF_UNKNOWN_PARAM_TEXT);
	}
	else if ((p->applied.mru_pitch_bias > GSF_MIN_PARAM) && (p->applied.mru_pitch_bias < GSF_MAX_PARAM)) {
		sprintf(temp, "APPLIED_MRU_PITCH=%+06.2f", p->applied.mru_pitch_bias);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (p->applied.mru_heading_bias == GSF_UNKNOWN_PARAM_VALUE) {
		sprintf(temp, "APPLIED_MRU_HEADING=%s", GSF_UNKNOWN_PARAM_TEXT);
	}
	else if ((p->applied.mru_heading_bias > GSF_MIN_PARAM) && (p->applied.mru_heading_bias < GSF_MAX_PARAM)) {
		sprintf(temp, "APPLIED_MRU_HEADING=%+06.2f", p->applied.mru_heading_bias);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (p->applied.mru_x_offset == GSF_UNKNOWN_PARAM_VALUE) {
		...
		sprintf(temp2, "%+06.2f,", p->applied.mru_x_offset);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (p->applied.mru_y_offset == GSF_UNKNOWN_PARAM_VALUE) {
		...
		sprintf(temp2, "%+06.2f,", p->applied.mru_y_offset);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (p->applied.mru_z_offset == GSF_UNKNOWN_PARAM_VALUE) {
		...
		sprintf(temp2, "%+06.2f", p->applied.mru_z_offset);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	else if ((p->applied.center_of_rotation_x_offset > GSF_MIN_PARAM) &&
	         (p->applied.center_of_rotation_x_offset < GSF_MAX_PARAM)) {
		sprintf(temp2, "%+06.2f,", p->applied.center_of_rotation_x_offset);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	else if ((p->applied.center_of_rotation_y_offset > GSF_MIN_PARAM) &&
	         (p->applied.center_of_rotation_y_offset < GSF_MAX_PARAM)) {
		sprintf(temp2, "%+06.2f,", p->applied.center_of_rotation_y_offset);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	else if ((p->applied.center_of_rotation_z_offset > GSF_MIN_PARAM) &&
	         (p->applied.center_of_rotation_z_offset < GSF_MAX_PARAM)) {
		sprintf(temp2, "%+06.2f", p->applied.center_of_rotation_z_offset);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (p->applied.position_latency == GSF_UNKNOWN_PARAM_VALUE) {
		sprintf(temp, "APPLIED_POSITION_LATENCY=%s", GSF_UNKNOWN_PARAM_TEXT);
	}
	else if ((p->applied.position_latency > GSF_MIN_PARAM) && (p->applied.position_latency < GSF_MAX_PARAM)) {
		sprintf(temp, "APPLIED_POSITION_LATENCY=%+06.3f", p->applied.position_latency);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (p->applied.attitude_latency == GSF_UNKNOWN_PARAM_VALUE) {
		sprintf(temp, "APPLIED_ATTITUDE_LATENCY=%s", GSF_UNKNOWN_PARAM_TEXT);
	}
	else if ((p->applied.attitude_latency > GSF_MIN_PARAM) && (p->applied.attitude_latency < GSF_MAX_PARAM)) {
		sprintf(temp, "APPLIED_ATTITUDE_LATENCY=%+06.3f", p->applied.attitude_latency);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (p->applied.depth_sensor_latency == GSF_UNKNOWN_PARAM_VALUE) {
		sprintf(temp, "APPLIED_DEPTH_SENSOR_LATENCY=%s", GSF_UNKNOWN_PARAM_TEXT);
	}
	else if ((p->applied.depth_sensor_latency > GSF_MIN_PARAM) && (p->applied.depth_sensor_latency < GSF_MAX_PARAM)) {
		sprintf(temp, "APPLIED_DEPTH_SENSOR_LATENCY=%+06.3f", p->applied.depth_sensor_latency);
	}
	else {
		gsfError = GSF_PROCESS_PARAM_RECORD_ENCODE_FAILED;
		return (-1);
	...
	if (num_rx == 1) {
		...
		else if ((p->applied.rx_transducer_x_offset[0] > GSF_MIN_PARAM) &&
		         (p->applied.rx_transducer_x_offset[0] < GSF_MAX_PARAM)) {
		...
		else if ((p->applied.rx_transducer_y_offset[0] > GSF_MIN_PARAM) &&
		         (p->applied.rx_transducer_y_offset[0] < GSF_MAX_PARAM)) {
		...
		else if ((p->applied.rx_transducer_z_offset[0] > GSF_MIN_PARAM) &&
		         (p->applied.rx_transducer_z_offset[0] < GSF_MAX_PARAM)) {
		...
		else if ((p->applied.rx_transducer_x_offset[1] > GSF_MIN_PARAM) &&
		         (p->applied.rx_transducer_x_offset[1] < GSF_MAX_PARAM)) {
		...
		else if ((p->applied.rx_transducer_y_offset[1] > GSF_MIN_PARAM) &&
		         (p->applied.rx_transducer_y_offset[1] < GSF_MAX_PARAM)) {
		...
		else if ((p->applied.rx_transducer_z_offset[1] > GSF_MIN_PARAM) &&
		         (p->applied.rx_transducer_z_offset[1] < GSF_MAX_PARAM)) {
	...
	if (num_rx == 1) {
		...
		else if ((p->applied.rx_transducer_pitch_offset[0] > GSF_MIN_PARAM) &&
		         (p->applied.rx_transducer_pitch_offset[0] < GSF_MAX_PARAM)) {
		...
		else if ((p->applied.rx_transducer_pitch_offset[1] > GSF_MIN_PARAM) &&
		         (p->applied.rx_transducer_pitch_offset[1] < GSF_MAX_PARAM)) {
	...
	if (num_rx == 1) {
		...
		else if ((p->applied.rx_transducer_roll_offset[0] > GSF_MIN_PARAM) &&
		         (p->applied.rx_transducer_roll_offset[0] < GSF_MAX_PARAM)) {
		...
		else if ((p->applied.rx_transducer_roll_offset[1] > GSF_MIN_PARAM) &&
		         (p->applied.rx_transducer_roll_offset[1] < GSF_MAX_PARAM)) {
	...
	if (num_rx == 1) {
		...
		else if ((p->applied.rx_transducer_heading_offset[0] > GSF_MIN_PARAM) &&
		         (p->applied.rx_transducer_heading_offset[0] < GSF_MAX_PARAM)) {
		...
		else if ((p->applied.rx_transducer_heading_offset[1] > GSF_MIN_PARAM) &&
		         (p->applied.rx_transducer_heading_offset[1] < GSF_MAX_PARAM)) {
...
int gsfGetMBParams(const gsfRecords *rec, gsfMBParams *p, int *numArrays) {
	...
	*numArrays = 0;
	...
	for (i = 0; i < rec->process_parameters.number_parameters; i++) {
		 ...
		 */
		...
		else if (strncmp(rec->process_parameters.param[i], "HEAVE_REMOVED_FROM_GPS_TC", strlen("HEAVE_REMOVED_FROM_GPS_TC")) ==
		         0) {
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "DRAFT_TO_APPLY=%lf,%lf", &p->to_apply.draft[0], &p->to_apply.draft[1]);
			}
			...
			*numArrays = gsfNumberParams(rec->process_parameters.param[i]);
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "PITCH_TO_APPLY=%lf,%lf", &p->to_apply.pitch_bias[0],
			...
			*numArrays = gsfNumberParams(rec->process_parameters.param[i]);
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "ROLL_TO_APPLY=%lf,%lf", &p->to_apply.roll_bias[0],
			...
			*numArrays = gsfNumberParams(rec->process_parameters.param[i]);
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "GYRO_TO_APPLY=%lf,%lf", &p->to_apply.gyro_bias[0],
		 ...
		 */
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "POSITION_OFFSET_TO_APPLY=%lf,%lf,%lf", &p->to_apply.position_x_offset,
				       &p->to_apply.position_y_offset, &p->to_apply.position_z_offset);
			}
		}
		else if (strncmp(rec->process_parameters.param[i], "TRANSDUCER_OFFSET_TO_APPLY", strlen("TRANSDUCER_OFFSET_TO_APPLY")) ==
		         0) {
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "TRANSDUCER_OFFSET_TO_APPLY=%lf,%lf,%lf,%lf,%lf,%lf",
				       &p->to_apply.transducer_x_offset[0], &p->to_apply.transducer_y_offset[0],
				       &p->to_apply.transducer_z_offset[0], &p->to_apply.transducer_x_offset[1],
				       &p->to_apply.transducer_y_offset[1], &p->to_apply.transducer_z_offset[1]);
			}
		}
		else if (strncmp(rec->process_parameters.param[i], "TRANSDUCER_PITCH_OFFSET_TO_APPLY",
		                 strlen("TRANSDUCER_PITCH_OFFSET_TO_APPLY")) == 0) {
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "TRANSDUCER_PITCH_OFFSET_TO_APPLY=%lf,%lf",
				       &p->to_apply.transducer_pitch_offset[0], &p->to_apply.transducer_pitch_offset[1]);
			}
			/* Get the number of array pairs from each sonar alignment parameter */
			*numArrays = gsfNumberParams(rec->process_parameters.param[i]);
			if (!num_tx)
		...
		else if (strncmp(rec->process_parameters.param[i], "TRANSDUCER_ROLL_OFFSET_TO_APPLY",
		                 strlen("TRANSDUCER_ROLL_OFFSET_TO_APPLY")) == 0) {
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "TRANSDUCER_ROLL_OFFSET_TO_APPLY=%lf,%lf",
				       &p->to_apply.transducer_roll_offset[0], &p->to_apply.transducer_roll_offset[1]);
			}
			/* Get the number of array pairs from each sonar alignment parameter */
			*numArrays = gsfNumberParams(rec->process_parameters.param[i]);
			if (!num_tx)
		...
		else if (strncmp(rec->process_parameters.param[i], "TRANSDUCER_HEADING_OFFSET_TO_APPLY",
		                 strlen("TRANSDUCER_HEADING_OFFSET_TO_APPLY")) == 0) {
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "TRANSDUCER_HEADING_OFFSET_TO_APPLY=%lf,%lf",
				       &p->to_apply.transducer_heading_offset[0], &p->to_apply.transducer_heading_offset[1]);
			}
			/* Get the number of array pairs from each sonar alignment parameter */
			*numArrays = gsfNumberParams(rec->process_parameters.param[i]);
			if (!num_tx)
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "MRU_PITCH_TO_APPLY=%lf", &p->to_apply.mru_pitch_bias);
			}
		}
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "MRU_ROLL_TO_APPLY=%lf", &p->to_apply.mru_roll_bias);
			}
		}
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "MRU_HEADING_TO_APPLY=%lf", &p->to_apply.mru_heading_bias);
			}
		}
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "MRU_OFFSET_TO_APPLY=%lf,%lf,%lf", &p->to_apply.mru_x_offset,
				       &p->to_apply.mru_y_offset, &p->to_apply.mru_z_offset);
			}
		}
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "ANTENNA_OFFSET_TO_APPLY=%lf,%lf,%lf", &p->to_apply.antenna_x_offset,
				       &p->to_apply.antenna_y_offset, &p->to_apply.antenna_z_offset);
			}
		}
		else if (strncmp(rec->process_parameters.param[i], "CENTER_OF_ROTATION_OFFSET_TO_APPLY",
		                 strlen("CENTER_OF_ROTATION_OFFSET_TO_APPLY")) == 0) {
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "CENTER_OF_ROTATION_OFFSET_TO_APPLY=%lf,%lf,%lf",
				       &p->to_apply.center_of_rotation_x_offset, &p->to_apply.center_of_rotation_y_offset,
				       &p->to_apply.center_of_rotation_z_offset);
			}
		}
		else if (strncmp(rec->process_parameters.param[i], "POSITION_LATENCY_TO_APPLY", strlen("POSITION_LATENCY_TO_APPLY")) ==
		         0) {
			p->to_apply.position_latency = GSF_UNKNOWN_PARAM_VALUE;
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "POSITION_LATENCY_TO_APPLY=%lf", &p->to_apply.position_latency);
			}
		}
		else if (strncmp(rec->process_parameters.param[i], "ATTITUDE_LATENCY_TO_APPLY", strlen("ATTITUDE_LATENCY_TO_APPLY")) ==
		         0) {
			p->to_apply.attitude_latency = GSF_UNKNOWN_PARAM_VALUE;
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "ATTITUDE_LATENCY_TO_APPLY=%lf", &p->to_apply.attitude_latency);
			}
		}
		else if (strncmp(rec->process_parameters.param[i], "DEPTH_SENSOR_LATENCY_TO_APPLY",
		                 strlen("DEPTH_SENSOR_LATENCY_TO_APPLY")) == 0) {
			p->to_apply.depth_sensor_latency = GSF_UNKNOWN_PARAM_VALUE;
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "DEPTH_SENSOR_LATENCY_TO_APPLY=%lf", &p->to_apply.depth_sensor_latency);
			}
		}
		else if (strncmp(rec->process_parameters.param[i], "DEPTH_SENSOR_OFFSET_TO_APPLY",
		                 strlen("DEPTH_SENSOR_OFFSET_TO_APPLY")) == 0) {
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "DEPTH_SENSOR_OFFSET_TO_APPLY=%lf,%lf,%lf",
				       &p->to_apply.depth_sensor_x_offset, &p->to_apply.depth_sensor_y_offset,
				       &p->to_apply.depth_sensor_z_offset);
			}
		}
		else if (strncmp(rec->process_parameters.param[i], "RX_TRANSDUCER_OFFSET_TO_APPLY",
		                 strlen("RX_TRANSDUCER_OFFSET_TO_APPLY")) == 0) {
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "RX_TRANSDUCER_OFFSET_TO_APPLY=%lf,%lf,%lf,%lf,%lf,%lf",
				       &p->to_apply.rx_transducer_x_offset[0], &p->to_apply.rx_transducer_y_offset[0],
				       &p->to_apply.rx_transducer_z_offset[0], &p->to_apply.rx_transducer_x_offset[1],
				       &p->to_apply.rx_transducer_y_offset[1], &p->to_apply.rx_transducer_z_offset[1]);
			}
		}
		else if (strncmp(rec->process_parameters.param[i], "RX_TRANSDUCER_PITCH_OFFSET_TO_APPLY",
		                 strlen("RX_TRANSDUCER_PITCH_OFFSET_TO_APPLY")) == 0) {
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "RX_TRANSDUCER_PITCH_OFFSET_TO_APPLY=%lf,%lf",
				       &p->to_apply.rx_transducer_pitch_offset[0], &p->to_apply.rx_transducer_pitch_offset[1]);
			}
			if (!num_rx)
				num_rx = gsfNumberParams(rec->process_parameters.param[i]);
		}
		else if (strncmp(rec->process_parameters.param[i], "RX_TRANSDUCER_ROLL_OFFSET_TO_APPLY",
		                 strlen("RX_TRANSDUCER_ROLL_OFFSET_TO_APPLY")) == 0) {
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "RX_TRANSDUCER_ROLL_OFFSET_TO_APPLY=%lf,%lf",
				       &p->to_apply.rx_transducer_roll_offset[0], &p->to_apply.rx_transducer_roll_offset[1]);
			}
			if (!num_rx)
				num_rx = gsfNumberParams(rec->process_parameters.param[i]);
		}
		else if (strncmp(rec->process_parameters.param[i], "RX_TRANSDUCER_HEADING_OFFSET_TO_APPLY",
		                 strlen("RX_TRANSDUCER_HEADING_OFFSET_TO_APPLY")) == 0) {
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "RX_TRANSDUCER_HEADING_OFFSET_TO_APPLY=%lf,%lf",
				       &p->to_apply.rx_transducer_heading_offset[0], &p->to_apply.rx_transducer_heading_offset[1]);
			}
			if (!num_rx)
				num_rx = gsfNumberParams(rec->process_parameters.param[i]);
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "APPLIED_DRAFT=%lf,%lf", &p->applied.draft[0], &p->applied.draft[1]);
			}
			/* Get the number of array pairs from each sonar alignment parameter */
			*numArrays = gsfNumberParams(rec->process_parameters.param[i]);
			if (!num_tx)
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "APPLIED_PITCH_BIAS=%lf,%lf", &p->applied.pitch_bias[0],
			...
			*numArrays = gsfNumberParams(rec->process_parameters.param[i]);
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "APPLIED_ROLL_BIAS=%lf,%lf", &p->applied.roll_bias[0],
			...
			*numArrays = gsfNumberParams(rec->process_parameters.param[i]);
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "APPLIED_GYRO_BIAS=%lf,%lf", &p->applied.gyro_bias[0],
		 ...
		 */
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "APPLIED_POSITION_OFFSET=%lf,%lf,%lf", &p->applied.position_x_offset,
				       &p->applied.position_y_offset, &p->applied.position_z_offset);
			}
		}
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "APPLIED_ANTENNA_OFFSET=%lf,%lf,%lf", &p->applied.antenna_x_offset,
				       &p->applied.antenna_y_offset, &p->applied.antenna_z_offset);
			}
		}
		/* The APPLIED_TRANSDUCER_OFFSET parameter defines the x,y,z offsets
		 * in ship coordinates to which have been applied to refer the x,y,z
		 * beam values to the ship reference point.
		 */
		else if (strncmp(rec->process_parameters.param[i], "APPLIED_TRANSDUCER_OFFSET", strlen("APPLIED_TRANSDUCER_OFFSET")) ==
		         0) {
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "APPLIED_TRANSDUCER_OFFSET=%lf,%lf,%lf,%lf,%lf,%lf",
				       &p->applied.transducer_x_offset[0], &p->applied.transducer_y_offset[0], &p->applied.transducer_z_offset[0],
				       &p->applied.transducer_x_offset[1], &p->applied.transducer_y_offset[1],
				       &p->applied.transducer_z_offset[1]);
			}
		}
		else if (strncmp(rec->process_parameters.param[i], "APPLIED_TRANSDUCER_PITCH_OFFSET",
		                 strlen("APPLIED_TRANSDUCER_PITCH_OFFSET")) == 0) {
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "APPLIED_TRANSDUCER_PITCH_OFFSET=%lf,%lf",
				       &p->applied.transducer_pitch_offset[0], &p->applied.transducer_pitch_offset[1]);
			}
			/* Get the number of array pairs from each sonar alignment parameter */
			*numArrays = gsfNumberParams(rec->process_parameters.param[i]);
			if (!num_tx)
		...
		else if (strncmp(rec->process_parameters.param[i], "APPLIED_TRANSDUCER_ROLL_OFFSET",
		                 strlen("APPLIED_TRANSDUCER_ROLL_OFFSET")) == 0) {
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "APPLIED_TRANSDUCER_ROLL_OFFSET=%lf,%lf",
				       &p->applied.transducer_roll_offset[0], &p->applied.transducer_roll_offset[1]);
			}
			/* Get the number of array pairs from each sonar alignment parameter */
			*numArrays = gsfNumberParams(rec->process_parameters.param[i]);
			if (!num_tx)
		...
		else if (strncmp(rec->process_parameters.param[i], "APPLIED_TRANSDUCER_HEADING_OFFSET",
		                 strlen("APPLIED_TRANSDUCER_HEADING_OFFSET")) == 0) {
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "APPLIED_TRANSDUCER_HEADING_OFFSET=%lf,%lf",
				       &p->applied.transducer_heading_offset[0], &p->applied.transducer_heading_offset[1]);
			}
			/* Get the number of array pairs from each sonar alignment parameter */
			*numArrays = gsfNumberParams(rec->process_parameters.param[i]);
			if (!num_tx)
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "APPLIED_MRU_PITCH=%lf", &p->applied.mru_pitch_bias);
			}
		}
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "APPLIED_MRU_ROLL=%lf", &p->applied.mru_roll_bias);
			}
		}
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "APPLIED_MRU_HEADING=%lf", &p->applied.mru_heading_bias);
			}
		}
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "APPLIED_MRU_OFFSET=%lf,%lf,%lf", &p->applied.mru_x_offset,
				       &p->applied.mru_y_offset, &p->applied.mru_z_offset);
			}
		}
		else if (strncmp(rec->process_parameters.param[i], "APPLIED_CENTER_OF_ROTATION_OFFSET",
		                 strlen("APPLIED_CENTER_OF_ROTATION_OFFSET")) == 0) {
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "APPLIED_CENTER_OF_ROTATION_OFFSET=%lf,%lf,%lf",
				       &p->applied.center_of_rotation_x_offset, &p->applied.center_of_rotation_y_offset,
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "APPLIED_POSITION_LATENCY=%lf", &p->applied.position_latency);
			}
		}
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "APPLIED_ATTITUDE_LATENCY=%lf", &p->applied.attitude_latency);
			}
		}
		else if (strncmp(rec->process_parameters.param[i], "APPLIED_DEPTH_SENSOR_LATENCY",
		                 strlen("APPLIED_DEPTH_SENSOR_LATENCY")) == 0) {
			p->applied.depth_sensor_latency = GSF_UNKNOWN_PARAM_VALUE;
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "APPLIED_DEPTH_SENSOR_LATENCY=%lf", &p->applied.depth_sensor_latency);
			}
		}
		else if (strncmp(rec->process_parameters.param[i], "APPLIED_DEPTH_SENSOR_OFFSET",
		                 strlen("APPLIED_DEPTH_SENSOR_OFFSET")) == 0) {
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "APPLIED_DEPTH_SENSOR_OFFSET=%lf,%lf,%lf",
				       &p->applied.depth_sensor_x_offset, &p->applied.depth_sensor_y_offset, &p->applied.depth_sensor_z_offset);
			}
		}
		else if (strncmp(rec->process_parameters.param[i], "APPLIED_RX_TRANSDUCER_OFFSET",
		                 strlen("APPLIED_RX_TRANSDUCER_OFFSET")) == 0) {
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "APPLIED_RX_TRANSDUCER_OFFSET=%lf,%lf,%lf,%lf,%lf,%lf",
				       &p->applied.rx_transducer_x_offset[0], &p->applied.rx_transducer_y_offset[0],
				       &p->applied.rx_transducer_z_offset[0], &p->applied.rx_transducer_x_offset[1],
				       &p->applied.rx_transducer_y_offset[1], &p->applied.rx_transducer_z_offset[1]);
			}
		}
		else if (strncmp(rec->process_parameters.param[i], "APPLIED_RX_TRANSDUCER_PITCH_OFFSET",
		                 strlen("APPLIED_RX_TRANSDUCER_PITCH_OFFSET")) == 0) {
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "APPLIED_RX_TRANSDUCER_PITCH_OFFSET=%lf,%lf",
				       &p->applied.rx_transducer_pitch_offset[0], &p->applied.rx_transducer_pitch_offset[1]);
			}
		}
		else if (strncmp(rec->process_parameters.param[i], "APPLIED_RX_TRANSDUCER_ROLL_OFFSET",
		                 strlen("APPLIED_RX_TRANSDUCER_ROLL_OFFSET")) == 0) {
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "APPLIED_RX_TRANSDUCER_ROLL_OFFSET=%lf,%lf",
				       &p->applied.rx_transducer_roll_offset[0], &p->applied.rx_transducer_roll_offset[1]);
			}
		}
		else if (strncmp(rec->process_parameters.param[i], "APPLIED_RX_TRANSDUCER_HEADING_OFFSET",
		                 strlen("APPLIED_RX_TRANSDUCER_HEADING_OFFSET")) == 0) {
			...
			if (!strstr(rec->process_parameters.param[i], GSF_UNKNOWN_PARAM_TEXT)) {
				sscanf(rec->process_parameters.param[i], "APPLIED_RX_TRANSDUCER_HEADING_OFFSET=%lf,%lf",
				       &p->applied.rx_transducer_heading_offset[0], &p->applied.rx_transducer_heading_offset[1]);
			}
		 ...
		 */
			...
			sscanf(rec->process_parameters.param[i], "TIDAL_DATUM=%s", str);
 ...
 *
 ********************************************************************/
int gsfLoadDepthScaleFactorAutoOffset(gsfSwathBathyPing *ping, unsigned int subrecordID, int reset, double min_depth,
                                      double max_depth, double *last_corrector, char c_flag, double precision) {
	double offset;
	double fraction;
	double layer;
	double next_layer;
	...
	if ((subrecordID != GSF_SWATH_BATHY_SUBRECORD_DEPTH_ARRAY) &&
	    (subrecordID != GSF_SWATH_BATHY_SUBRECORD_NOMINAL_DEPTH_ARRAY)) {
...
void gsfInitializeMBParams(gsfMBParams *p) {
	...
	p->msb_applied_to_attitude =
	    GSF_UNKNOWN_PARAM_INT; // Changed from GSF_UNKNOWN_PARAM_VALUE for MB-System DW Caress 21 March 2017
	p->heave_removed_from_gps_tc =
	    GSF_UNKNOWN_PARAM_INT; // Changed from GSF_UNKNOWN_PARAM_VALUE for MB-System DW Caress 21 March 2017
	p->utc_offset = GSF_UNKNOWN_PARAM_INT;
	p->roll_reference = GSF_UNKNOWN_PARAM_INT;
	p->number_of_transmitters = GSF_UNKNOWN_PARAM_INT;
