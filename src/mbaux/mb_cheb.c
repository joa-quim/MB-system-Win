 ...
 *    $Id: mb_cheb.c 2283 2016-10-23 09:33:10Z caress $
 *
...
*/
void
mbcblas_dcopy( const int N, const double *X,
             const int incX, double *Y, const int incY)
{
  int i;
  int ix = MBCBLAS_OFFSET(N, incX);
...
*/
double
mbcblas_ddot( const int N, const double *X,
...
// ---------------------------------------------------------------------
static double
mblsqr_d2norm( const double a, const double b )
{
    double scale;
...
static void
mblsqr_dload( const int n, const double alpha, double x[] )
{    
    int i;
    ...
    goto_400:
        if ( extra ) {
            ...
            fprintf(nout, " %6d %16.9e %16.9e %9.2e %9.2e %8.1e %8.1e  Convergence: %9.2e %9.2e\n",
                    itn, x[0], rnorm, test1, test2, anorm, acond, test1/rtol, test2/atol);
        }
 ...
 goto_800:
    ...
    if (nout != NULL) {
        fprintf(nout,   "\n"
                ...
                exit, bnorm, xnorm,
                exit, rnorm, arnorm);
        fprintf(nout,   " %s       max dx = %7.1e occured at itn %-9d\n"
                        " %s              = %7.1e*xnorm\n",
                exit, dxmax, maxdx,
