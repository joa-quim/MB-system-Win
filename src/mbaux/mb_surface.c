 ...
 *    $Id: mb_surface.c 2308 2017-06-04 19:55:48Z caress $
 *
...
static char rcs_id[] = "$Id: mb_surface.c 2308 2017-06-04 19:55:48Z caress $";
...
int mb_surface(int verbose, int ndat, float *xdat, float *ydat, float *zdat, double xxmin, double xxmax, double yymin,
	...
	*/
	...
	status = mb_mallocd(local_verbose, __FILE__, __LINE__, mx * my * sizeof(char), (void **)&iu, &local_error);
	status = mb_mallocd(local_verbose, __FILE__, __LINE__, mx * my * sizeof(float), (void **)&u, &local_error);
...
void check_errors() {
	...
	for (k = 0; k < npoints; k++) {
		...
		d2u_dxdy = 0.25 * (u[ij + move_over[3]] - u[ij + move_over[1]] - u[ij + move_over[10]] + u[ij + move_over[8]]);
		d3u_dx3 = 0.5 * (u[ij + move_over[7]] - 2 * u[ij + move_over[6]] + 2 * u[ij + move_over[5]] - u[ij + move_over[4]]);
		d3u_dy3 = 0.5 * (u[ij + move_over[0]] - 2 * u[ij + move_over[2]] + 2 * u[ij + move_over[9]] - u[ij + move_over[11]]);
...
void load_constraints(char *low, char *high) {
	...
	if (set_low > 0) {
		status = mb_mallocd(local_verbose, __FILE__, __LINE__, nx * ny * sizeof(float), (void **)&lower, &local_error);
		if (set_low < 3)
			for (i = 0; i < nx * ny; i++)
				lower[i] = low_limit;
		/* Comment this out:
	...
	if (set_high > 0) {
		status = mb_mallocd(local_verbose, __FILE__, __LINE__, nx * ny * sizeof(float), (void **)&upper, &local_error);
		if (set_high < 3)
			for (i = 0; i < nx * ny; i++)
				upper[i] = high_limit;
		/* Comment this out:
