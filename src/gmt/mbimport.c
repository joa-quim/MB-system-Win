...
GMT_LOCAL int mbimport_plot_box(struct CTRL *Ctrl, struct GMT_CTRL *GMT, double *x, double *y, double *rgb, int *error);
GMT_LOCAL int mbimport_plot_point(struct CTRL *Ctrl, struct GMT_CTRL *GMT, double x, double y, double *rgb, int *error);
...
GMT_LOCAL int parse (struct GMT_CTRL *GMT, struct CTRL *Ctrl, struct GMT_OPTION *options) {
	...
	if (!GMT->common.R.active) {	/* When no Region specified, set a fake one here */
	...
	n_errors += gmt_M_check_condition (GMT, Ctrl->I.active && !Ctrl->I.inputfile, "Syntax error -I option: Must specify input file\n");
...
int GMT_mbimport (void *V_API, int mode, void *args) {
	...
	if (Ctrl->C.active) {
		...
		char   file[8] = {""};	/* If used as is, it will mean 'rainbow' to gmt_get_cpt() */
		double zmin, zmax;
		...
		if ((CPTcolor = gmt_get_cpt(GMT, file, GMT_CPT_OPTIONAL, zmin, zmax)) == NULL)
			Return (API->error);
		if (CPTcolor->is_gray && Ctrl->image_type == MBSWATH_IMAGE_24)
			Ctrl->image_type = MBSWATH_IMAGE_8;
	}
	...
	if (Ctrl->read_datalist == MB_YES) {
		...
		if ((status = mb_datalist_read(verbose, Ctrl->datalist, file, &format, &Ctrl->file_weight, &error)) == MB_SUCCESS)
			read_data = true;
		else
			read_data = false;
	}
	...
	if ((I = GMT_Create_Data (API, GMT_IS_IMAGE, GMT_IS_SURFACE, GMT_GRID_HEADER_ONLY, dim, GMT->common.R.wesn, xy_inc, 0, 0, NULL)) == NULL) {
	...
	if (GMT_Write_Data (API, GMT_IS_IMAGE, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_ALL, NULL, Ctrl->M.file, I) != GMT_OK)
		return EXIT_FAILURE;
		...
		for (i=first;i<first+nplot;i++) {
			...
			for (j=0;j<pingcur->beams_bath;j++) {
				if (pingcur->bathflag[j] == MB_YES) {
					...
					status = mbimport_plot_box(Ctrl, GMT, xx, yy, rgb, error);
				}
			}
		}
	}
		...
		for (i = first; i < first + nplot; i++) {		/* loop over all pings and beams and plot the good ones */
			...
			for (j=0;j<pingcur->beams_bath;j++) {
				if (mb_beam_ok(pingcur->beamflag[j])) {
					...
					if (Ctrl->Z.mode == MBSWATH_BATH_RELIEF || Ctrl->Z.mode == MBSWATH_BATH_AMP)
						gmt_illuminate(GMT, pingcur->bathshade[j], rgb);
					status = mbimport_plot_point(Ctrl, GMT, xx, yy, rgb, error);
				}
			}
		}
	}
