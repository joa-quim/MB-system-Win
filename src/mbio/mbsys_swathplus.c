 ...
 *	$Id: mbsys_swathplus.c 2308 2017-06-04 19:55:48Z caress $
 *
...
static char rcs_id[] = "$Id: mbsys_swathplus.c 2308 2017-06-04 19:55:48Z caress $";
...
int mbsys_swathplus_deall(int verbose, void *mbio_ptr, void **store_ptr, int *error) {
	...
	if (sxp_ping->points != NULL) {
		mb_freed(verbose, __FILE__, __LINE__, (void **)&(sxp_ping->points), error);
		sxp_ping->points_alloc = 0;
	}
	if (sxi_ping->sampnum != NULL) {
		mb_freed(verbose, __FILE__, __LINE__, (void **)&(sxi_ping->sampnum), error);
		mb_freed(verbose, __FILE__, __LINE__, (void **)&(sxi_ping->angle), error);
		mb_freed(verbose, __FILE__, __LINE__, (void **)&(sxi_ping->amplitude), error);
		mb_freed(verbose, __FILE__, __LINE__, (void **)&(sxi_ping->quality), error);
		sxi_ping->samps_alloc = 0;
	}
	if (projection->projection_id != NULL) {
		mb_freed(verbose, __FILE__, __LINE__, (void **)&(projection->projection_id), error);
		projection->projection_alloc = 0;
	}
	if (comment->message != NULL) {
		mb_freed(verbose, __FILE__, __LINE__, (void **)&(comment->message), error);
...
int mbsys_swathplus_extract(int verbose, void *mbio_ptr, void *store_ptr, int *kind, int time_i[7], double *time_d,
	...
	*kind = store->kind;
	...
	if ((*kind == MB_DATA_DATA) && ((type == SWPLS_ID_PROCESSED_PING) || (type == SWPLS_ID_PROCESSED_PING2))) {
		...
		*speed = 0.0;
		if (mb_io_ptr->nfix > 0) {
			mb_navint_prjinterp(verbose, mbio_ptr, store->time_d, *heading, *speed, navlon, navlat, speed, error);
		}
		...
		*namp = *nbath;
		...
		swpls_concat_translate(verbose, &wtov, -(sxp_ping->txer_e), 0.0, -(sxp_ping->txer_n), error);
	...
	*kind = store->kind;
	...
	if ((*kind == MB_DATA_DATA) && ((type == SWPLS_ID_PROCESSED_PING) || (type == SWPLS_ID_PROCESSED_PING2))) {
		...
		*draft = 0.0;
		...
		swpls_init_transform(verbose, &wtov, error);
		swpls_concat_translate(verbose, &wtov, -sxp_ping->txer_e, -(-(sxp_ping->height - sxp_ping->tide)), -sxp_ping->txer_n,
		                       error);
...
int mbsys_swathplus_insert_nav(int verbose, void *mbio_ptr, void *store_ptr, int time_i[7], double time_d, double navlon,
	...
	if ((kind == MB_DATA_DATA) && ((type == SWPLS_ID_PROCESSED_PING) || (type == SWPLS_ID_PROCESSED_PING2))) {
		...
		swpls_concat_translate(verbose, &m, -(sxp_ping->txer_e), -(-(sxp_ping->height - sxp_ping->tide)), -sxp_ping->txer_n,
		                       error);
		swpls_concat_translate(verbose, &m, -old_txoffset.x, -old_txoffset.y, -old_txoffset.z, error);
		...
		swpls_concat_translate(verbose, &m, new_txoffset.x, new_txoffset.y, new_txoffset.z, error);
...
int mbsys_swathplus_copy(int verbose, void *mbio_ptr, void *store_ptr, void *copy_ptr, int *error) {
	...
	if ((status == MB_SUCCESS) && (copy->sxi_ping.samps_alloc < store->sxi_ping.samps_alloc)) {
		status = mb_reallocd(verbose, __FILE__, __LINE__, store->sxi_ping.samps_alloc * sizeof(unsigned short),
		                     (void **)&(copy->sxi_ping.sampnum), error);
		if (status == MB_SUCCESS) {
			status = mb_reallocd(verbose, __FILE__, __LINE__, store->sxi_ping.samps_alloc * sizeof(short int),
			                     (void **)&(copy->sxi_ping.angle), error);
		}
		if (status == MB_SUCCESS) {
			status = mb_reallocd(verbose, __FILE__, __LINE__, store->sxi_ping.samps_alloc * sizeof(unsigned short),
			                     (void **)&(copy->sxi_ping.amplitude), error);
		}
		if (status == MB_SUCCESS) {
			status = mb_reallocd(verbose, __FILE__, __LINE__, store->sxi_ping.samps_alloc * sizeof(unsigned char),
			                     (void **)&(copy->sxi_ping.quality), error);
		}
		if (status == MB_SUCCESS) {
	...
	if (status == MB_SUCCESS) {
		memcpy(copy->sxi_ping.sampnum, store->sxi_ping.sampnum, store->sxi_ping.samps_alloc * sizeof(unsigned short));
		memcpy(copy->sxi_ping.angle, store->sxi_ping.angle, store->sxi_ping.samps_alloc * sizeof(short int));
		memcpy(copy->sxi_ping.amplitude, store->sxi_ping.amplitude, store->sxi_ping.samps_alloc * sizeof(unsigned short));
		memcpy(copy->sxi_ping.quality, store->sxi_ping.quality, store->sxi_ping.samps_alloc * sizeof(unsigned char));
	}
	...
	if (copy->projection.projection_alloc < store->projection.projection_alloc) {
		status = mb_reallocd(verbose, __FILE__, __LINE__, store->projection.projection_alloc,
	...
	if (status == MB_SUCCESS) {
		strncpy(copy->projection.projection_id, store->projection.projection_id, store->projection.projection_alloc);
	}
	...
	if (copy->comment.message_alloc < store->comment.message_alloc) {
		status = mb_reallocd(verbose, __FILE__, __LINE__, store->comment.message_alloc, (void **)&(copy->comment.message), error);
		if (status == MB_SUCCESS) {
	...
	if (status == MB_SUCCESS) {
		strncpy(copy->comment.message, store->comment.message, store->comment.message_alloc);
	}
...
int swpls_chk_header(int verbose, void *mbio_ptr, char *buffer, int *recordid, int *size, int *error) {
	...
	/* get values to check */
	index = 0;
	mb_get_binary_int(MB_YES, &buffer[index], recordid);
	index += 4;
	...
	if ((*recordid != SWPLS_ID_SXP_HEADER_DATA) && (*recordid != SWPLS_ID_PROCESSED_PING) &&
	    (*recordid != SWPLS_ID_PROCESSED_PING2) && (*recordid != SWPLS_ID_COMMENT) && (*recordid != SWPLS_ID_PROJECTION) &&
	    (*recordid != SWPLS_ID_SXI_HEADER_DATA) && (*recordid != SWPLS_ID_PARSED_PING) &&
	    (*recordid != SWPLS_ID_PARSED_ATTITUDE) && (*recordid != SWPLS_ID_PARSED_POSITION_LL) &&
	    (*recordid != SWPLS_ID_PARSED_POSITION_EN) && (*recordid != SWPLS_ID_PARSED_SSV) &&
	    (*recordid != SWPLS_ID_PARSED_ECHOSOUNDER) && (*recordid != SWPLS_ID_PARSED_TIDE) &&
	    (*recordid != SWPLS_ID_PARSED_TIDE) && (*recordid != SWPLS_ID_PARSED_AGDS) && (*recordid != SWPLS_ID_POS_OFFSET) &&
	    (*recordid != SWPLS_ID_IMU_OFFSET) && (*recordid != SWPLS_ID_TXER_OFFSET) && (*recordid != SWPLS_ID_WL_OFFSET)) {
		status = MB_FAILURE;
...
int swpls_rd_sxpping(int verbose, char *buffer, void *store_ptr, int pingtype, int *error) {
	...
	mb_get_binary_int(MB_YES, &buffer[index], &int_val);
	index += 4;
	ping->pingnumber = (unsigned int)int_val;
	index += 4; /* padding bytes */
	mb_get_binary_double(MB_YES, &buffer[index], &(ping->time_d));
	index += 8;
	mb_get_binary_int(MB_YES, &buffer[index], &(ping->notxers));
	index += 4;
	index += 4; /* padding bytes */
	mb_get_binary_double(MB_YES, &buffer[index], &(ping->easting));
	index += 8;
	mb_get_binary_double(MB_YES, &buffer[index], &(ping->northing));
	index += 8;
	mb_get_binary_double(MB_YES, &buffer[index], &(ping->roll));
	index += 8;
	mb_get_binary_double(MB_YES, &buffer[index], &(ping->pitch));
	index += 8;
	mb_get_binary_double(MB_YES, &buffer[index], &(ping->heading));
	index += 8;
	mb_get_binary_double(MB_YES, &buffer[index], &(ping->height));
	index += 8;
	mb_get_binary_double(MB_YES, &buffer[index], &(ping->tide));
	index += 8;
	mb_get_binary_double(MB_YES, &buffer[index], &(ping->sos));
	index += 8;
	...
	index += 8;
	mb_get_binary_short(MB_YES, &buffer[index], &(ping->trnstime));
	index += 2;
	mb_get_binary_short(MB_YES, &buffer[index], &(ping->recvtime));
	index += 2;
	...
	index += 4;
	index += 4; /* padding bytes */
	mb_get_binary_double(MB_YES, &buffer[index], &(ping->txer_e));
	index += 8;
	mb_get_binary_double(MB_YES, &buffer[index], &(ping->txer_n));
	index += 8;
	...
	if (status == MB_SUCCESS) {
		for (i = 0; i < ping->nosampsfile; i++) {
			...
			if (pingtype == SWPLS_ID_PROCESSED_PING) {
				...
				mb_get_binary_double(MB_YES, &buffer[index], &(ping->points[i].tpu));
				index += 8;
			}
		}
	}
...
int swpls_rd_projection(int verbose, char *buffer, void *store_ptr, int *error) {
	...
	if (store->projection_set == MB_NO) {
		...
		if (status == MB_SUCCESS) {
			...
			store->time_d = (double)projection->time_d + (double)projection->microsec * 1e-6;
...
int swpls_rd_comment(int verbose, char *buffer, void *store_ptr, int *error) {
	...
	if (comment->message_alloc < comment->nchars) {
		status = mb_reallocd(verbose, __FILE__, __LINE__, (size_t)comment->nchars, (void **)&(comment->message), error);
		if (status != MB_SUCCESS) {
			comment->message_alloc = 0;
		}
		else {
	...
	if (status == MB_SUCCESS) {
		/* set the date and time */
		store->time_d = (double)comment->time_d + (double)comment->microsec * 1e-6;
		mb_get_date(verbose, store->time_d, &(store->time_i[0]));
...
int swpls_rd_sxiping(int verbose, char *buffer, void *store_ptr, int *error) {
	...
	mb_get_binary_float(MB_YES, &buffer[index], &(ping->sos));
	index += 4;
	mb_get_binary_short(MB_YES, &buffer[index], &(ping->txpulse));
	index += 2;
	ping->data_options = buffer[index];
	index++;
	ping->ping_state = (unsigned char)buffer[index];
	index++;
	mb_get_binary_short(MB_YES, &buffer[index], &short_val);
	index += 2;
	...
	if (ping->samps_alloc < ping->nosamps) {
		...
		if (status == MB_SUCCESS) {
			...
			status = mb_reallocd(verbose, __FILE__, __LINE__, read_len, (void **)&(ping->angle), error);
		}
		if (status == MB_SUCCESS) {
			read_len = (size_t)(ping->nosamps * sizeof(unsigned short));
			status = mb_reallocd(verbose, __FILE__, __LINE__, read_len, (void **)&(ping->amplitude), error);
		}
		if (status == MB_SUCCESS) {
			read_len = (size_t)(ping->nosamps * sizeof(unsigned char));
			status = mb_reallocd(verbose, __FILE__, __LINE__, read_len, (void **)&(ping->quality), error);
		}
		if (status != MB_SUCCESS) {
			ping->samps_alloc = 0;
		}
	...
	if (status == MB_SUCCESS) {
		for (i = 0; i < ping->nosamps; i++) {
			...
			ping->quality[i] = (unsigned char)buffer[index];
			index++;
		}
	}
...
int swpls_rd_attitude(int verbose, char *buffer, void *store_ptr, int *error) {
	...
	index += 4;
	attitude->channel = buffer[index];
	index++;
	mb_get_binary_float(MB_YES, &buffer[index], &(attitude->roll));
	index += 4;
...
int swpls_rd_agds(int verbose, char *buffer, void *store_ptr, int *error) {
	...
	index += 4;
	agds->channel = buffer[index];
	index++;
	mb_get_binary_float(MB_YES, &buffer[index], &(agds->hardness));
	index += 4;
...
int swpls_rd_pos_offset(int verbose, char *buffer, void *store_ptr, int *error) {
	...
	index += 4;
	pos_offset->channel = buffer[index];
	index++;
...
int swpls_rd_imu_offset(int verbose, char *buffer, void *store_ptr, int *error) {
	...
	index += 4;
	imu_offset->channel = buffer[index];
	index++;
...
int swpls_rd_txer_offset(int verbose, char *buffer, void *store_ptr, int *error) {
	...
	index += 4;
	txer_offset->channel = buffer[index];
	index++;
...
int swpls_pr_posll(int verbose, FILE *fout, swpls_posll *posll, int *error) {
	...
	fprintf(fout, "\n%s  SWPLS_ID_PARSED_POSITION_LL [ID: 0x%X] %d bytes\n", starter, SWPLS_ID_PARSED_POSITION_LL,
	        SWPLS_SIZE_POSITION_LL);
...
int swpls_pr_posen(int verbose, FILE *fout, swpls_posen *posen, int *error) {
	...
	fprintf(fout, "\n%s  SWPLS_ID_PARSED_POSITION_EN [ID: 0x%X] %d bytes\n", starter, SWPLS_ID_PARSED_POSITION_EN,
	        SWPLS_SIZE_POSITION_EN);
...
int swpls_pr_ssv(int verbose, FILE *fout, swpls_ssv *ssv, int *error) {
	...
	fprintf(fout, "\n%s  SWPLS_ID_PARSED_SSV [ID: 0x%X] %d bytes\n", starter, SWPLS_ID_PARSED_SSV, SWPLS_SIZE_SSV);
...
int swpls_pr_agds(int verbose, FILE *fout, swpls_agds *agds, int *error) {
	...
	fprintf(fout, "\n%s  SWPLS_ID_PARSED_AGDS [ID: 0x%X] %d bytes\n", starter, SWPLS_ID_PARSED_AGDS, SWPLS_SIZE_AGDS);
...
int swpls_pr_txer_offset(int verbose, FILE *fout, swpls_txer_offset *txer_offset, int *error) {
	...
	fprintf(fout, "\n%s  SWPLS_ID_TXER_OFFSET [ID: 0x%X] %d bytes\n", starter, SWPLS_ID_TXER_OFFSET, SWPLS_SIZE_TXER_OFFSET);
	fprintf(fout, "%s        time_d:       %d :: %s", starter, txer_offset->time_d, asctime(gmtime(&tm)));
	fprintf(fout, "%s        microsec:     %d\n", starter, txer_offset->microsec);
	fprintf(fout, "%s        channel:      %u\n", starter, txer_offset->channel);
	fprintf(fout, "%s        height:       %f\n", starter, txer_offset->height);
	fprintf(fout, "%s        forward:      %f\n", starter, txer_offset->forward);
	fprintf(fout, "%s        starboard:    %f\n", starter, txer_offset->starboard);
	fprintf(fout, "%s        azimuth:      %f\n", starter, txer_offset->azimuth);
	fprintf(fout, "%s        elevation:    %f\n", starter, txer_offset->elevation);
...
int swpls_pr_wl_offset(int verbose, FILE *fout, swpls_wl_offset *wl_offset, int *error) {
	...
	fprintf(fout, "\n%s  SWPLS_ID_WL_OFFSET [ID: 0x%X] %d bytes\n", starter, SWPLS_ID_WL_OFFSET, SWPLS_SIZE_WL_OFFSET);
