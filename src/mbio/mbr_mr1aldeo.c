 ...
 *	$Id: mbr_mr1aldeo.c 2308 2017-06-04 19:55:48Z caress $
 *
...
static char rcs_id[] = "$Id: mbr_mr1aldeo.c 2308 2017-06-04 19:55:48Z caress $";
...
int mbr_info_mr1aldeo(int verbose, int *system, int *beams_bath_max, int *beams_amp_max, int *pixels_ss_max, char *format_name,
	...
	*pixels_ss_max = 7003;
	...
	strncpy(format_description,
	        "Format name:          MBF_MR1ALDEO\nInformal Description: L-DEO MR1 post processed format with travel "
...
int mbr_mr1aldeo_rd_data(int verbose, void *mbio_ptr, int *error) {
	...
	if (mb_io_ptr->fileheader == MB_NO) {
		status = mbr_mr1aldeo_rd_hdr(verbose, xdrs, data, &mb_io_ptr->hdr_comment, error);
...
int mbr_mr1aldeo_rd_ping(int verbose, XDR *xdrs, struct mbf_mr1aldeo_struct *data, int *error) {
	...
	if (data->port_btycount > MBF_MR1ALDEO_BEAMS_SIDE) {
		...
		dummy_count = data->port_btycount - MBF_MR1ALDEO_BEAMS_SIDE;
		data->port_btycount = MBF_MR1ALDEO_BEAMS_SIDE;
	}
	else
		dummy_count = 0;
	...
	if (data->port_sscount > MBF_MR1ALDEO_PIXELS_SIDE) {
		...
		dummy_count = data->port_sscount - MBF_MR1ALDEO_PIXELS_SIDE;
		data->port_sscount = MBF_MR1ALDEO_PIXELS_SIDE;
	}
	else
		dummy_count = 0;
	...
	if (data->stbd_btycount > MBF_MR1ALDEO_BEAMS_SIDE) {
		...
		dummy_count = data->stbd_btycount - MBF_MR1ALDEO_BEAMS_SIDE;
	...
	if (data->stbd_sscount > MBF_MR1ALDEO_PIXELS_SIDE) {
		...
		dummy_count = data->stbd_sscount - MBF_MR1ALDEO_PIXELS_SIDE;
...
int mbr_mr1aldeo_wr_data(int verbose, void *mbio_ptr, struct mbf_mr1aldeo_struct *data, int *error) {
	...
	/* if data and file header not written */
		...
		/* write file header */
		status = mbr_mr1aldeo_wr_hdr(verbose, xdrs, data, &mb_io_ptr->hdr_comment, error);
		mb_io_ptr->fileheader = MB_YES;
