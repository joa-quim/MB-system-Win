 ...
 *	$Id: mbr_oicmbari.c 2308 2017-06-04 19:55:48Z caress $
 *
...
static char rcs_id[] = "$Id: mbr_oicmbari.c 2308 2017-06-04 19:55:48Z caress $";
...
int mbr_rt_oicmbari(int verbose, void *mbio_ptr, void *store_ptr, int *error) {
	...
	while (status == MB_SUCCESS && (buffer[0] != 'G' || buffer[1] != 'E' || buffer[2] != '2')) {
		for (i = 0; i < 3; i++)
			buffer[i] = buffer[i + 1];
	...
	if (status == MB_SUCCESS) {
		if ((read_size = fread(&buffer[4], 1, MBF_OICMBARI_HEADER_SIZE - 4, mb_io_ptr->mbfp)) != MBF_OICMBARI_HEADER_SIZE - 4) {
			status = MB_FAILURE;
			*error = MB_ERROR_EOF;
		}
	}
...
/* byte swap the data if necessary */
	...
	if (status == MB_SUCCESS && dataplus->kind == MB_DATA_DATA) {
		...
		if (header->beams_bath > data->beams_bath_alloc || data->bath == NULL) {
			...
			if (data->angle != NULL)
				status = mb_freed(verbose, __FILE__, __LINE__, (void **)&(data->angle), error);
			status =
			    mb_mallocd(verbose, __FILE__, __LINE__, header->beams_bath * sizeof(char), (void **)&(data->beamflag), error);
	...
	if (status == MB_SUCCESS && dataplus->kind == MB_DATA_DATA && header->beams_bath > 0) {
		/* get beamflag array */
	...
	if (status == MB_SUCCESS && dataplus->kind == MB_DATA_DATA && header->beams_amp > 0) {
		/* get amplitude array */
	...
	if (status == MB_SUCCESS && dataplus->kind == MB_DATA_DATA && header->pixels_ss > 0) {
		/* get ss array */
...
int mbr_wt_oicmbari(int verbose, void *mbio_ptr, void *store_ptr, int *error) {
	...
	if (store != NULL) {
		...
		/* depths and sidescan */
		if (header->beams_bath > data->beams_bath_alloc || data->beamflag == NULL || data->bath == NULL ||
		    data->bathacrosstrack == NULL || data->bathalongtrack == NULL || data->tt == NULL || data->angle == NULL) {
			data->beams_bath_alloc = header->beams_bath;
			if (data->beamflag != NULL)
				status = mb_freed(verbose, __FILE__, __LINE__, (void **)&(data->beamflag), error);
			if (data->bath != NULL)
			...
			if (data->angle != NULL)
				status = mb_freed(verbose, __FILE__, __LINE__, (void **)&(data->angle), error);
			status =
			    mb_mallocd(verbose, __FILE__, __LINE__, data->beams_bath_alloc * sizeof(char), (void **)&(data->beamflag), error);
			status =
	...
	if ((write_size = fwrite(buffer, 1, MBF_OICMBARI_HEADER_SIZE, mb_io_ptr->mbfp)) == MBF_OICMBARI_HEADER_SIZE) {
		status = MB_SUCCESS;
		*error = MB_ERROR_NO_ERROR;
	}
	else {
