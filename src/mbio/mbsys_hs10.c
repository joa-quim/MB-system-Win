 ...
 *	$Id: mbsys_hs10.c 2308 2017-06-04 19:55:48Z caress $
 *
...
static char rcs_id[] = "$Id: mbsys_hs10.c 2308 2017-06-04 19:55:48Z caress $";
	...
	if (*kind == MB_DATA_DATA || *kind == MB_DATA_CALIBRATE) {
		...
		*nbeams = MBSYS_HS10_BEAMS;
		...
		for (i = 0; i < mb_io_ptr->beams_bath_max; i++) {
			ttimes[i] = 0.0;
			angles[i] = (i - MBSYS_HS10_BEAMS / 2) * MBSYS_HS10_BEAMSPACING;
			if (angles[i] < 0.0) {
				angles[i] = -angles[i];
