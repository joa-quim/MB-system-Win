 ...
 *	$Id: mbr_dsl120pf.c 2308 2017-06-04 19:55:48Z caress $
 *
...
static char rcs_id[] = "$Id: mbr_dsl120pf.c 2308 2017-06-04 19:55:48Z caress $";
...
int mbr_info_dsl120pf(int verbose, int *system, int *beams_bath_max, int *beams_amp_max, int *pixels_ss_max, char *format_name,
	...
	*pixels_ss_max = 8192;
	...
	strncpy(format_description,
	        ...
	        "2048 beam bathymetry, 8192 pixel sidescan,\n                      binary, parallel bathymetry and amplitude files, "
	        "WHOI DSL.\n",
...
int mbr_wt_dsl120pf(int verbose, void *mbio_ptr, void *store_ptr, int *error) {
	...
	if (store != NULL) {
		...
		strncpy(data->comment, store->comment, MBF_DSL120PF_COMMENT_LENGTH - 1);
	}
...
int mbr_dsl120pf_rd_data(int verbose, void *mbio_ptr, int *error) {
	...
	if (mb_io_ptr->mbfp != NULL) {
		...
		if (status == MB_SUCCESS)
			status = mbr_dsl120pf_rd_header(verbose, mbio_ptr, mb_io_ptr->mbfp, error);

		/* now read each of the data records */
		if (status == MB_SUCCESS)
			for (i = 0; i < data->num_data_types; i++) {
				status = mbr_dsl120pf_rd_dataheader(verbose, mbio_ptr, mb_io_ptr->mbfp, type, &len, &hdr_len, error);

				if (status == MB_SUCCESS && strncmp(type, "BATH", 4) == 0) {
					data->bat_len = len;
					data->bat_hdr_len = hdr_len;
					status = mbr_dsl120pf_rd_bath(verbose, mbio_ptr, mb_io_ptr->mbfp, error);
					if (status == MB_SUCCESS)
					...
					status = mbr_dsl120pf_rd_amp(verbose, mbio_ptr, mb_io_ptr->mbfp, error);
					if (status == MB_SUCCESS)
						data->kind = MB_DATA_DATA;
				}
				else if (status == MB_SUCCESS && strncmp(type, "COMM", 4) == 0) {
					status = mbr_dsl120pf_rd_comment(verbose, mbio_ptr, mb_io_ptr->mbfp, error);
					if (status == MB_SUCCESS)
	...
	/* read second file */
	if (status == MB_SUCCESS && data->kind == MB_DATA_DATA && mb_io_ptr->mbfp2 != NULL) {
		/* read next four bytes */
		found = MB_NO;
		...
		/* now read the rest of the header */
		if (status == MB_SUCCESS)
			status = mbr_dsl120pf_rd_header(verbose, mbio_ptr, mb_io_ptr->mbfp2, error);

		/* now read each of the data records */
		if (status == MB_SUCCESS)
			for (i = 0; i < data->num_data_types; i++) {
				status = mbr_dsl120pf_rd_dataheader(verbose, mbio_ptr, mb_io_ptr->mbfp2, type, &len, &hdr_len, error);

				if (status == MB_SUCCESS && strncmp(type, "BATH", 4) == 0) {
					data->bat_len = len;
					data->bat_hdr_len = hdr_len;
					status = mbr_dsl120pf_rd_bath(verbose, mbio_ptr, mb_io_ptr->mbfp2, error);
					if (status == MB_SUCCESS)
						data->kind = MB_DATA_DATA;
				}
				else if (status == MB_SUCCESS && strncmp(type, "AMP ", 4) == 0) {
					data->amp_len = len;
					data->amp_hdr_len = hdr_len;
					status = mbr_dsl120pf_rd_amp(verbose, mbio_ptr, mb_io_ptr->mbfp2, error);
					if (status == MB_SUCCESS)
						data->kind = MB_DATA_DATA;
				}
				else if (status == MB_SUCCESS && strncmp(type, "COMM", 4) == 0) {
					status = mbr_dsl120pf_rd_comment(verbose, mbio_ptr, mb_io_ptr->mbfp2, error);
					if (status == MB_SUCCESS)
						data->kind = MB_DATA_COMMENT;
				}
			}
...
int mbr_dsl120pf_wr_data(int verbose, void *mbio_ptr, char *data_ptr, int *error) {
	...
	if (data->kind == MB_DATA_COMMENT) {
		...
		if (data->bat_num_bins > 0)
			status = mbr_dsl120pf_wr_bath(verbose, mbio_ptr, mb_io_ptr->mbfp, error);
		else if (data->amp_num_samp > 0)
			status = mbr_dsl120pf_wr_amp(verbose, mbio_ptr, mb_io_ptr->mbfp, error);
		if (mb_io_ptr->mbfp2 != NULL && data->amp_num_samp > 0)
			status = mbr_dsl120pf_wr_amp(verbose, mbio_ptr, mb_io_ptr->mbfp2, error);
	}
	else {
		status = MB_FAILURE;
