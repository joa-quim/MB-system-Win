 ...
 *	$Id: mbr_xtfb1624.c 2308 2017-06-04 19:55:48Z caress $
 *
...
static char rcs_id[] = "$Id: mbr_xtfb1624.c 2308 2017-06-04 19:55:48Z caress $";
		...
		/* read rest of sidescan packet */
			...
			if (status == MB_SUCCESS && read_len == 64) {
				...
				if (pingchanportheader->TimeDuration == 0.0) {
					pingchanportheader->TimeDuration = pingchanportheader->SlantRange / pingheader->SoundVelocity;
				}
			}
			else {
				status = MB_FAILURE;
			...
			*/
			...
			if (status == MB_SUCCESS && read_len == 64) {
				...
				if (pingchanstbdheader->TimeDuration == 0.0) {
					pingchanstbdheader->TimeDuration = pingchanstbdheader->SlantRange / pingheader->SoundVelocity;
				}
			}
			else {
				status = MB_FAILURE;
			...
			*/
			...
			if (verbose >= 5) {
				...
				fprintf(stderr, "dbg5       MagicNumber:                %d %d %x%x\n", pingheader->packetheader.MagicNumber[0],
				        pingheader->packetheader.MagicNumber[1], pingheader->packetheader.MagicNumber[0],
				        pingheader->packetheader.MagicNumber[1]);
				fprintf(stderr, "dbg5       HeaderType:                 %d\n", pingheader->packetheader.HeaderType);
				fprintf(stderr, "dbg5       SubChannelNumber:           %d\n", pingheader->packetheader.SubChannelNumber);
				fprintf(stderr, "dbg5       NumChansToFollow:           %d\n", pingheader->packetheader.NumChansToFollow);
				fprintf(stderr, "dbg5       Reserved1:                  %d %d\n", pingheader->packetheader.Reserved1[0],
				        pingheader->packetheader.Reserved1[1]);
