 ...
 *	$Id: mbr_photgram.c 2308 2017-06-04 19:55:48Z caress $
 *
...
static char rcs_id[] = "$Id: mbr_photgram.c 2308 2017-06-04 19:55:48Z caress $";
...
int mbr_alm_photgram(int verbose, void *mbio_ptr, int *error) {
	...
	status = mbsys_stereopair_alloc(verbose, mbio_ptr, &mb_io_ptr->store_data, error);
...
int mbr_photgram_rd_data(int verbose, void *mbio_ptr, void *store_ptr, int *error) {
	...
	if (status == MB_SUCCESS) {
		...
		if (strncmp(&buffer[4], "DDPG", 4) == 0) {
			...
			mb_get_binary_double(swap, &buffer[index], &store->time_d);
			index += 8;
			mb_get_date(verbose, store->time_d, store->time_i);
			mb_get_binary_double(swap, &buffer[index], &store->longitude);
			index += 8;
			mb_get_binary_double(swap, &buffer[index], &store->latitude);
			index += 8;
			mb_get_binary_double(swap, &buffer[index], &store->sensordepth);
			index += 8;
			mb_get_binary_float(swap, &buffer[index], &store->heading);
			index += 4;
			mb_get_binary_float(swap, &buffer[index], &store->roll);
			index += 4;
			mb_get_binary_float(swap, &buffer[index], &store->pitch);
			index += 4;
			mb_get_binary_float(swap, &buffer[index], &store->speed);
			index += 4;
			mb_get_binary_float(swap, &buffer[index], &store->altitude);
			index += 4;
			mb_get_binary_int(swap, &buffer[index], &store->num_soundings);
			index += 4;
			...
			for (i = 0; i < store->num_soundings; i++) {
				...
				/* parse the sounding */
				index = 0;
				mb_get_binary_double(swap, &buffer[index], &sounding->acrosstrack);
				index += 8;
				mb_get_binary_double(swap, &buffer[index], &sounding->alongtrack);
				index += 8;
				mb_get_binary_double(swap, &buffer[index], &sounding->depth);
				index += 8;
				sounding->beamflag = buffer[index];
				index++;
				sounding->red = buffer[index];
				index++;
				sounding->green = buffer[index];
				index++;
				sounding->blue = buffer[index];
				index++;
...
int mbr_photgram_wr_data(int verbose, void *mbio_ptr, void *store_ptr, int *error) {
	...
	if (status == MB_SUCCESS) {
		...
		if (store->kind == MB_DATA_DATA) {
			...
			index = 0;
			mb_put_binary_int(swap, write_len, (void *)&buffer[index]);
			index += 4;
			strncpy(&buffer[index], "DDPG", 4);
			index += 4;
			mb_put_binary_double(swap, store->time_d, (void *)&buffer[index]);
			index += 8;
			mb_put_binary_double(swap, store->longitude, (void *)&buffer[index]);
			index += 8;
			mb_put_binary_double(swap, store->latitude, (void *)&buffer[index]);
			index += 8;
			mb_put_binary_double(swap, store->sensordepth, (void *)&buffer[index]);
			index += 8;
			mb_put_binary_float(swap, store->heading, (void *)&buffer[index]);
			index += 4;
			mb_put_binary_float(swap, store->roll, (void *)&buffer[index]);
			index += 4;
			mb_put_binary_float(swap, store->pitch, (void *)&buffer[index]);
			index += 4;
			mb_put_binary_float(swap, store->speed, (void *)&buffer[index]);
			index += 4;
			mb_put_binary_float(swap, store->altitude, (void *)&buffer[index]);
			index += 4;
			mb_put_binary_int(swap, store->num_soundings, (void *)&buffer[index]);
			index += 4;
			write_len = index;
			...
			for (i = 0; i < store->num_soundings; i++) {
				...
				sounding = &store->soundings[i];
				index = 0;
				mb_put_binary_double(swap, sounding->acrosstrack, (void *)&buffer[index]);
				index += 8;
				mb_put_binary_double(swap, sounding->alongtrack, (void *)&buffer[index]);
				index += 8;
				mb_put_binary_double(swap, sounding->depth, (void *)&buffer[index]);
				index += 8;
				buffer[index] = sounding->beamflag;
				index++;
				buffer[index] = sounding->red;
				index++;
				buffer[index] = sounding->green;
				index++;
				buffer[index] = sounding->blue;
				index++;
				write_len = index;
				status = mb_fileio_put(verbose, mbio_ptr, buffer, &write_len, error);
			}
			...
			index = 0;
			strncpy(&buffer[index], "END!", 4);
			index += 4;
			mb_put_binary_short(swap, checksum, (void *)&buffer[index]);
			index += 2;
			write_len = index;
			status = mb_fileio_put(verbose, mbio_ptr, buffer, &write_len, error);
		}
		...
		/* write a comment record */
			...
			mb_put_binary_int(swap, write_len, (void *)&buffer[index]);
			index += 4;
			strncpy(&buffer[index], "DDCM", 4);
			index += 4;
			mb_put_binary_int(swap, store->comment_len, (void *)&buffer[index]);
			index += 4;
			strncpy(&buffer[index], store->comment, store->comment_len);
