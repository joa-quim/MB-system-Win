...
int mbr_rt_xtfr8101(int verbose, void *mbio_ptr, void *store_ptr, int *error) {
	...
	if (status == MB_SUCCESS && data->kind == MB_DATA_DATA) {
		...
		dtime = 3600.0 * (data->bathheader.FixTimeHour - data->bathheader.Hour) +
		        60.0 * (data->bathheader.FixTimeMinute - data->bathheader.Minute) + data->bathheader.FixTimeSecond -
		        data->bathheader.Second - 0.01 * data->bathheader.HSeconds;
	...
	timetag = 0.001 * data->bathheader.AttitudeTimeTag
			...
			timetag = 0.001 * data->bathheader.AttitudeTimeTag - store->png_latency +
			...
			fprintf(stderr, "roll: %d %f %f %f %f   latency:%f time:%f %f roll:%f\n", mb_io_ptr->nattitude,
			        ...
			        (double)(0.001 * data->bathheader.AttitudeTimeTag), timetag, store->png_roll);
#endif
		}
		else {
		...
		mb_lever(verbose, (double)store->MBOffsetX, (double)store->MBOffsetY, (double)store->MBOffsetZ, (double)store->NavOffsetX,
		         (double)store->NavOffsetY, (double)store->NavOffsetZ, (double)store->MRUOffsetX, (double)store->MRUOffsetY,
		         (double)store->MRUOffsetZ, (double)(store->png_roll - store->MRUOffsetRoll),
		         (double)(store->MRUOffsetPitch - store->png_pitch), &lever_x, &lever_y, &lever_z, error);
		store->png_heave -= lever_z;
#ifdef MBR_XTFR8101_DEBUG
		fprintf(stderr, "offsets: %f %f %f   roll:%f pitch:%f    dz:%f\n", store->MBOffsetX - store->MRUOffsetX,
		        store->MBOffsetY - store->MRUOffsetY, store->MBOffsetZ - store->MRUOffsetZ,
		        (double)(store->png_roll - store->MRUOffsetRoll), (double)(store->MRUOffsetPitch - store->png_pitch), lever_z);
#endif
	...
	while (status == MB_SUCCESS && done == MB_NO) {
		...
		if (read_len == 12) {
			...
			if (verbose >= 5) {
				...
				fprintf(stderr, "dbg5       MagicNumber:                %d %d %x%x\n", packetheader.MagicNumber[0],
				        packetheader.MagicNumber[1], packetheader.MagicNumber[0], packetheader.MagicNumber[1]);
				...
				fprintf(stderr, "dbg5       Reserved1:                  %d %d\n", packetheader.Reserved1[0],
				        packetheader.Reserved1[1]);
		...
		if (status == MB_SUCCESS && packetheader.HeaderType == XTF_DATA_ATTITUDE && packetheader.NumBytesThisRecord == 64) {
#ifdef MBR_XTFR8101_DEBUG
			...
			if (read_len == 50) {
				...
				if (verbose >= 5) {
					...
					fprintf(stderr, "dbg5       Reserved1:                  %d %d\n", attitudeheader->packetheader.Reserved1[0],
					        attitudeheader->packetheader.Reserved1[1]);
					fprintf(stderr, "dbg5       NumBytesThisRecord:         %d\n",
		...
		/* read rest of sidescan packet */
		else if (status == MB_SUCCESS && packetheader.HeaderType == XTF_DATA_SIDESCAN) {
#ifdef MBR_XTFR8101_DEBUG
			...
			if (status == MB_SUCCESS && read_len == 64) {
				...
				if (pingchanportheader->TimeDuration == 0.0) {
					pingchanportheader->TimeDuration = pingchanportheader->SlantRange / sidescanheader->SoundVelocity;
				}
			}
			else {
				status = MB_FAILURE;
			...
			if (pingchanportheader->ChannelNumber >
			    (fileheader->NumberOfSonarChannels + fileheader->NumberOfBathymetryChannels - 1)) {
				status = MB_FAILURE;
				*error = MB_ERROR_UNINTELLIGIBLE;
			}
			else if (pingchanportheader->NumSamples > fileheader->chaninfo[pingchanportheader->ChannelNumber].SamplesPerChannel) {
				status = MB_FAILURE;
				*error = MB_ERROR_UNINTELLIGIBLE;
			}
			...
			if (status == MB_SUCCESS && read_len == 64) {
				...
				if (pingchanstbdheader->TimeDuration == 0.0) {
					pingchanstbdheader->TimeDuration = pingchanstbdheader->SlantRange / sidescanheader->SoundVelocity;
				}
			}
			else {
				status = MB_FAILURE;
			...
			else if (pingchanstbdheader->NumSamples > fileheader->chaninfo[pingchanstbdheader->ChannelNumber].SamplesPerChannel) {
				status = MB_FAILURE;
				*error = MB_ERROR_UNINTELLIGIBLE;
			}
			...
			if (status == MB_SUCCESS) {
				read_bytes =
				    pingchanstbdheader->NumSamples * fileheader->chaninfo[pingchanstbdheader->ChannelNumber].BytesPerSample;
				read_len = fread(line, 1, read_bytes, mb_io_ptr->mbfp);
			}
			if (status == MB_SUCCESS && read_len == read_bytes) {
			...
			if (verbose >= 5) {
				...
				fprintf(stderr, "dbg5       Reserved1:                  %d %d\n", sidescanheader->packetheader.Reserved1[0],
				        sidescanheader->packetheader.Reserved1[1]);
		...
		/* read rest of bathymetry packet */
		else if (status == MB_SUCCESS && packetheader.HeaderType == XTF_DATA_BATHYMETRY) {
#ifdef MBR_XTFR8101_DEBUG
			...
			if (read_len == 242) {
				...
				if (verbose >= 5) {
					...
					fprintf(stderr, "dbg5       Reserved1:                  %d %d\n", bathheader->packetheader.Reserved1[0],
					        bathheader->packetheader.Reserved1[1]);
					...
					for (i = 0; i < reson8100rit->beam_count; i++) {
						...
						fprintf(stderr, "  quality:%3.3d %d%d%d%d\n", quality, quality & 1, quality >> 1 & 1, quality >> 2 & 1,
						        quality >> 3 & 1);
					}
				}
