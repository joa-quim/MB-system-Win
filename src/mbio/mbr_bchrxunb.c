 ...
 *	$Id: mbr_bchrxunb.c 2308 2017-06-04 19:55:48Z caress $
 *
...
static char rcs_id[] = "$Id: mbr_bchrxunb.c 2308 2017-06-04 19:55:48Z caress $";
...
/* swap bytes if necessary */
		...
		*type = (short int)mb_swap_short(*type);
		...
		if (status == MB_FAILURE) {
			...
			status = mbr_bchrxunb_rd_comment(verbose, mbfp, data, error);
			...
			status = mbr_bchrxunb_rd_parameter(verbose, mbfp, data, error);
			if (status == MB_SUCCESS) {
				done = MB_YES;
			...
			status = mbr_bchrxunb_rd_pos(verbose, mbfp, data, error);
			if (status == MB_SUCCESS) {
				done = MB_YES;
			...
			status = mbr_bchrxunb_rd_svp(verbose, mbfp, data, error);
			if (status == MB_SUCCESS) {
				done = MB_YES;
			...
			status = mbr_bchrxunb_rd_bath56(verbose, mbfp, data, error);
			if (status == MB_SUCCESS) {
				done = MB_YES;
			...
			status = mbr_bchrxunb_rd_bath40(verbose, mbfp, data, error);
			if (status == MB_SUCCESS) {
				done = MB_YES;
				data->kind = MB_DATA_DATA;
			}
			...
			status = mbr_bchrxunb_rd_bath32(verbose, mbfp, data, error);
			if (status == MB_SUCCESS) {
				done = MB_YES;
				data->kind = MB_DATA_DATA;
			}
	...
	if (status == MB_SUCCESS) {
		...
		for (i = 0; i < data->profile_num; i++) {
			...
			for (j = 0; j < 8; j++) {
				...
				int_ptr = (int *)&beam[4];
				data->profile[i].bath_acrosstrack[j] = (int)*int_ptr;
			...
			for (j = 0; j < 8; j++) {
				...
				int_ptr = (int *)&beam[4];
				data->profile[i].bath_acrosstrack[j] = (int)mb_swap_int(*int_ptr);
				short_ptr = (short *)&beam[8];
			...
			*short_ptr = (short int)data->profile[i].heave;
			for (j = 0; j < 8; j++) {
				...
				*int_ptr = (int)data->profile[i].bath[j];
				int_ptr = (int *)&beam[4];
				*int_ptr = (int)data->profile[i].bath_acrosstrack[j];
				short_ptr = (short *)&beam[8];
			...
			*short_ptr = (short int)mb_swap_short((short int)data->profile[i].heave);
			for (j = 0; j < 8; j++) {
				...
				*int_ptr = (int)mb_swap_int(data->profile[i].bath[j]);
				int_ptr = (int *)&beam[4];
				*int_ptr = (int)mb_swap_int(data->profile[i].bath_acrosstrack[j]);
				short_ptr = (short *)&beam[8];
