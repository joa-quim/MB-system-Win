 ...
 *	$Id: mbsys_jstar.c 2308 2017-06-04 19:55:48Z caress $
 *
...
static char svn_id[] = "$Id: mbsys_jstar.c 2308 2017-06-04 19:55:48Z caress $";
...
int mbsys_jstar_extract_altitude(int verbose, void *mbio_ptr, void *store_ptr, int *kind, double *transducer_depth,
	...
	*kind = store->kind;
	...
	if (*kind == MB_DATA_SUBBOTTOM_SUBBOTTOM) {
		...
		if (sbp->sonardepth > 0)
			*transducer_depth = 0.001 * sbp->sonardepth + sbp->heaveCompensation * sbp->sampleInterval * 0.00000075;
		else
			*transducer_depth =
			    sbp->startDepth * sbp->sampleInterval * 0.00000075 + sbp->heaveCompensation * sbp->sampleInterval * 0.00000075;
		*altitude = 0.001 * sbp->sonaraltitude;
		...
		if (ssport->sonardepth > 0)
			*transducer_depth = 0.001 * ssport->sonardepth + ssport->heaveCompensation * ssport->sampleInterval * 0.00000075;
		else
...
int mbsys_jstar_extract_nav(int verbose, void *mbio_ptr, void *store_ptr, int *kind, int time_i[7], double *time_d,
	...
	*kind = store->kind;
	...
	if (*kind == MB_DATA_SUBBOTTOM_SUBBOTTOM) {
		...
		*heave = sbp->heaveCompensation * sbp->sampleInterval * 0.00000075;
	...
	/* extract data from structure */
		...
		*heave = ssport->heaveCompensation * ssport->sampleInterval * 0.00000075;
...
int mbsys_jstar_extract_segytraceheader(int verbose, void *mbio_ptr, void *store_ptr, int *kind, void *segytraceheader_ptr,
	...
	*kind = store->kind;
	...
	if (*kind == MB_DATA_SUBBOTTOM_SUBBOTTOM) {
		...
		/* get transducer_depth */
		if (sbp->sonardepth > 0)
			dsonardepth = 0.001 * sbp->sonardepth + sbp->heaveCompensation * sbp->sampleInterval * 0.00000075;
		else
			dsonardepth =
			    sbp->startDepth * sbp->sampleInterval * 0.00000075 + sbp->heaveCompensation * sbp->sampleInterval * 0.00000075;
