...
void RegisterBxConverters ARGLIST((appContext)) GRA(XtAppContext, appContext) {
	...
	XtAppSetTypeConverter(appContext, XmRString, XmRXmStringTable, (XtTypeConverter)&CvtStringToXmStringTable, NULL, 0,
	                      XtCacheNone, (XtDestructor)&CvtXmStringTableDestructor);
}
...
Pixmap XPM_PIXMAP ARGLIST((w, pixmapName)) ARG(Widget, w) GRA(char **, pixmapName) {
	...
	attributes.visual = DefaultVisual(XtDisplayOfObject(w), DefaultScreen(XtDisplayOfObject(w)));

	if (attributes.depth != 0)
		attributes.valuemask |= BxXpmDepth;
	if (attributes.colormap != 0)
		attributes.valuemask |= BxXpmColormap;
	if (attributes.visual != 0)
		attributes.valuemask |= BxXpmVisual;

	returnValue = BxXpmCreatePixmapFromData(XtDisplayOfObject(w), DefaultRootWindow(XtDisplayOfObject(w)), pixmapName, &pixmap,
	                                        &shape, &attributes);
	if (shape) {
...
void SetAppDefaults ARGLIST((w, defs, inst_name, override_inst)) ARG(Widget, w) ARG(UIAppDefault *, defs) ARG(char *, inst_name)
	...
	while (defs->wName != NULL) {
		...
		if (defs->value == NULL || (override_inst && strncmp(inst_name, defs->wName, namelen)) ||
		...
		if (defs->cInstName != NULL) {
			 ...
			 */
			if (*defs->cInstName != '\0') {
				sprintf(buf, "%s*%s*%s.%s: %s", appClass, defs->cInstName, defs->wName, defs->wRsc, defs->value);
			}
			else {
				sprintf(buf, "%s*%s.%s: %s", appClass, defs->wName, defs->wRsc, defs->value);
			}
		}
		else if (*defs->wName != '\0') {
			sprintf(buf, "%s*%s*%s.%s: %s", appClass, inst_name, defs->wName, defs->wRsc, defs->value);
		}
		else {
			sprintf(buf, "%s*%s.%s: %s", appClass, inst_name, defs->wRsc, defs->value);
		}
